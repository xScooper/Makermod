...
//	newb = (char *) G_Alloc2( l );
	newb = G_FindAlloc(buffer);//G_FindMemory( buffer );
	if (!newb)
		return G_NewString2(buffer);
...
Builds a copy of the string, translating \n to real linefeeds
so message texts can be multi-line
New version, fixing memory leak:
=============
*/
/*char *G_NewStringM( const char *string, int ID ) {
	...
	if (AllocPoints[ID].AllocPointer && (Allocated[ID] == 1))
		G_Free(ID);

	if (AllocPoints[ID].AllocPointer)
		AllocPoints[ID].AllocPointer = 0;

//	newb = (char *) G_Alloc( l );
	trap_TrueMalloc((void **)&AllocPoints[ID].AllocPointer, l);
	Allocated[ID] = 1;
	new_p = AllocPoints[ID].AllocPointer;
	//Q_strncpyz(DeAllocate[0][0],newb,sizeof(DeAllocate[0]));
	...
	return AllocPoints[ID].AllocPointer;
}*/

extern char* G_RequestAllocation ( int entityNum, int size );
char* G_NewStringForEnt ( const char* string, int entityNum )
{
	char* mem = NULL;
	char parsedText[MAX_STRING_CHARS] = { 0 };
	int i = 0, charCount = 0;
	
	while ( string[i] != 0 && charCount < (MAX_STRING_CHARS - 1) && i < MAX_STRING_CHARS )
	{
		if ( string[i] == '\\' && i < (MAX_STRING_CHARS - 1) )
		{
			if ( string[++i] == 'n' )
			{
				parsedText[charCount++] = '\n';
			}
			else
			{
				parsedText[charCount++] = '\\';
		...
		{
			parsedText[charCount++] = string[i];
		}
		
		++i;
	}
	parsedText[charCount] = 0;
	
	mem = G_RequestAllocation (entityNum, charCount);
	memcpy (mem, parsedText, charCount);
...
const char *SkpWhitespace( const char *data, qboolean *hasNewLines ) {
	int c;
	int com_lines = 0;
...
char *ParseExt( const char **data_p, qboolean allowLineBreaks )
{
	int c = 0, len;
	int com_lines = 0;
	...
	{
		// skip whitespace
		data = SkpWhitespace( data, &hasNewLines );
		if ( !data )
		{
			*data_p = NULL;
			return com_token;
...
void AddSpawnField(char *field, char *value)
{
	...
	if ( stricmp(field, "classname") == 0 ) // allow multi-class etities
	{
		// Make sure we don't repeat a classname
		for(i=0;i<level.numSpawnVars;i++)
		{
			if (Q_stricmp(level.spawnVars[i][1], value) == 0)
				return;
		}
	}
	else
	{
		// replace any other parameters 
		for(i=0;i<level.numSpawnVars;i++)
		{
