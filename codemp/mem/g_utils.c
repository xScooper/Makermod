...
//Enable for corpse dragging
	...
	{
		if (target && target->passworded && (!(Q_stricmp(target->objectPassword, ent->client->sess.objectPassword) == 0) ) )
		{
			trap_SendServerCommand( ent-g_entities, va("print \"This object is passworded.\n\""));
			return;
		}
	}
