...
// SPINMOD REMOVED
#ifndef CGAME
qboolean Script_ScaleMe_f(itemDef_t *item, char **args) {
		trap_Cmd_ExecuteText(EXEC_APPEND, va("mscaleme %f", DC->getCVarValue(item->cvar)));
	return qtrue;
}

qboolean Script_ListObs_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mlistobs %s", buffer));
	return qtrue;
}

qboolean Script_ScaleT_f(itemDef_t *item, char **args) {
		trap_Cmd_ExecuteText(EXEC_APPEND, va("mscalet %f", DC->getCVarValue(item->cvar)));
	return qtrue;
}

qboolean Script_Tele_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mtele %s", buffer));
	return qtrue;
}

qboolean Script_Select_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mselect %s", buffer));
	return qtrue;
}

qboolean Script_Arm_f(itemDef_t *item, char **args) {
		trap_Cmd_ExecuteText(EXEC_APPEND, va("marm %f", DC->getCVarValue(item->cvar)));
	return qtrue;
}


qboolean Script_Move1_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mmove %s %s 0", buffer, buffer));
	return qtrue;
}

qboolean Script_Move2_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mmove %s 0 0", buffer));
	return qtrue;
}

qboolean Script_Move3_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mmove %s -%s 0", buffer, buffer));
	return qtrue;
...
qboolean Script_Move4_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mmove 0 %s 0", buffer));
	return qtrue;
}

qboolean Script_Move5_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mmove 0 -%s 0", buffer));
	return qtrue;
...
qboolean Script_Move6_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mmove -%s %s 0", buffer, buffer));
	return qtrue;
...
qboolean Script_Move7_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mmove -%s 0 0", buffer));
	return qtrue;
...
qboolean Script_Move8_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mmove -%s -%s 0", buffer, buffer));
	return qtrue;
...
qboolean Script_MoveUp_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
	 trap_Cmd_ExecuteText(EXEC_APPEND, va("mmove 0 0 %s", buffer));
	return qtrue;
}

qboolean Script_MoveDwn_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mmove 0 0 -%s", buffer));
	return qtrue;
...
qboolean Script_MoveAll1_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mmoveall %s %s 0", buffer, buffer));
	return qtrue;
}

qboolean Script_MoveAll2_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mmoveall %s 0 0", buffer));
	return qtrue;
}

qboolean Script_MoveAll3_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mmoveall %s -%s 0", buffer, buffer));
	return qtrue;
...
qboolean Script_MoveAll4_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mmoveall 0 %s 0", buffer));
	return qtrue;
}

qboolean Script_MoveAll5_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mmoveall 0 -%s 0", buffer));
	return qtrue;
...
qboolean Script_MoveAll6_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mmoveall -%s %s 0", buffer, buffer));
	return qtrue;
...
qboolean Script_MoveAll7_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mmoveall -%s 0 0", buffer));
	return qtrue;
...
qboolean Script_MoveAll8_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mmoveall -%s -%s 0", buffer, buffer));
	return qtrue;
...
qboolean Script_MoveAllUp_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
	 trap_Cmd_ExecuteText(EXEC_APPEND, va("mmoveall 0 0 %s", buffer));
	return qtrue;
}

qboolean Script_MoveAllDwn_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mmoveall 0 0 -%s", buffer));
	return qtrue;
...
qboolean Script_Rotate1_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mrotate %s %s 0", buffer, buffer));
	return qtrue;
}

qboolean Script_Rotate2_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mrotate %s 0 0", buffer));
	return qtrue;
}

qboolean Script_Rotate3_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mrotate %s -%s 0", buffer, buffer));
	return qtrue;
...
qboolean Script_Rotate4_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mrotate 0 %s 0", buffer));
	return qtrue;
}

qboolean Script_Rotate5_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mrotate 0 -%s 0", buffer));
	return qtrue;
...
qboolean Script_Rotate6_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mrotate -%s %s 0", buffer, buffer));
	return qtrue;
...
qboolean Script_Rotate7_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mrotate -%s 0 0", buffer));
	return qtrue;
...
qboolean Script_Rotate8_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mrotate -%s -%s 0", buffer, buffer));
	return qtrue;
...
qboolean Script_RotateUp_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
	 trap_Cmd_ExecuteText(EXEC_APPEND, va("mrotate 0 0 %s", buffer));
	return qtrue;
}

qboolean Script_RotateDwn_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mrotate 0 0 -%s", buffer));
	return qtrue;
...
qboolean Script_PlaceFx_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mplacefx %s %s\n", uiInfo.efxList[uiInfo.efxIndex], buffer));
	return qtrue;
}


qboolean Script_Back1_f(itemDef_t *item) {
	menuDef_t *menu;
	menu = Menu_GetFocused();	
...
qboolean Script_Forward1_f(itemDef_t *item) {
	menuDef_t *menu;
	menu = Menu_GetFocused();	
	item = Menu_FindItemByName(menu, "mplace_01");
	return (qtrue);
}

qboolean Script_Pain_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mpain %s", buffer));
	return qtrue;
}

qboolean Script_BreakAble_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mbreakable %s", buffer));
	return qtrue;
}

qboolean Script_BreakAbleAll_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mbreakableall %s", buffer));
	return qtrue;
}

qboolean Script_Scale_f(itemDef_t *item, char **args) {
		trap_Cmd_ExecuteText(EXEC_APPEND, va("mscale %f", DC->getCVarValue(item->cvar)));
	return qtrue;
}

qboolean Script_SaveObs_f(itemDef_t *item, char **args) {
	 char ui_savename[MAX_QPATH];
	 char ui_savepass[MAX_QPATH];
	 trap_Cvar_VariableStringBuffer("ui_savename", ui_savename, sizeof(ui_savename) );
	 trap_Cvar_VariableStringBuffer("ui_savepass", ui_savepass, sizeof(ui_savepass) );
     trap_Cmd_ExecuteText(EXEC_APPEND, va("msaveobs %s %s", ui_savename, ui_savepass));
	return qtrue;
}

qboolean Script_LoadObs_f(itemDef_t *item, char **args) {
	 char ui_loadname[MAX_QPATH];
	 char ui_loadpass[MAX_QPATH];
	 trap_Cvar_VariableStringBuffer("ui_loadname", ui_loadname, sizeof(ui_loadname) );
	 trap_Cvar_VariableStringBuffer("ui_loadpass", ui_loadpass, sizeof(ui_loadpass) );
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mloadobs %s %s", ui_loadname, ui_loadpass));
	return qtrue;
}

qboolean Script_Unban_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("munban %s", buffer));
	return qtrue;
}

qboolean Script_NewUser_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mnewuser %s", buffer));
	 trap_Cmd_ExecuteText(EXEC_NOW , va("echo Plugin Confirms Creation of User ^1%s", buffer));
	return qtrue;
}

qboolean Script_ChangePass_f(itemDef_t *item, char **args) {
	 char ui_mchangepassoldpass[MAX_QPATH];
	 char ui_mchangepassnewpass1[MAX_QPATH];
	 char ui_mchangepassnewpass2[MAX_QPATH];
	 trap_Cvar_VariableStringBuffer("ui_mchangepassoldpass", ui_mchangepassoldpass, sizeof(ui_mchangepassoldpass) );
	 trap_Cvar_VariableStringBuffer("ui_mchangepassnewpass1", ui_mchangepassnewpass1, sizeof(ui_mchangepassnewpass1) );
	 trap_Cvar_VariableStringBuffer("ui_mchangepassnewpass2", ui_mchangepassnewpass2, sizeof(ui_mchangepassnewpass2) );
	 trap_Cmd_ExecuteText(EXEC_APPEND, va("mchangepass %s %s %s", ui_mchangepassoldpass, ui_mchangepassnewpass1, ui_mchangepassnewpass2));
	return qtrue;
}

qboolean Script_ResetPass_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mresetpass %s", buffer));
	return qtrue;
}

qboolean Script_Animate_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("manim %s %s", uiInfo.AnimList[uiInfo.AnimIndex], buffer));
	return qtrue;
}

qboolean Script_AnimateB_f(itemDef_t *item, char **args) {
     char buffer[100];
	 DC->getCVarString(item->cvar, buffer, sizeof(buffer));
	 trap_Cmd_ExecuteText( EXEC_APPEND, va("wait 500; manim %s %s\n", uiInfo.AnimList[uiInfo.AnimIndex], buffer));
    	return qtrue;
}

qboolean Script_Animate2_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("manim %s %s", uiInfo.Anim2List[uiInfo.Anim2Index], buffer));
	return qtrue;
}

qboolean Script_Animate2B_f(itemDef_t *item, char **args) {
     char buffer[100];
	 DC->getCVarString(item->cvar, buffer, sizeof(buffer));
	 trap_Cmd_ExecuteText( EXEC_APPEND, va("wait 500; manim %s %s\n", uiInfo.Anim2List[uiInfo.Anim2Index], buffer));
    	return qtrue;
}

qboolean Script_Animate3_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("manim %s %s", uiInfo.Anim3List[uiInfo.Anim3Index], buffer));
	return qtrue;
}

qboolean Script_Animate3B_f(itemDef_t *item, char **args) {
     char buffer[100];

	 DC->getCVarString(item->cvar, buffer, sizeof(buffer));
	 trap_Cmd_ExecuteText( EXEC_APPEND, va("wait 500; manim %s %s\n", uiInfo.Anim3List[uiInfo.Anim3Index], buffer));
    	return qtrue;
}

qboolean Script_Permissions_f(itemDef_t *item, char **args) {
     char buffer[100];
     DC->getCVarString(item->cvar, buffer, sizeof(buffer));
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mpermissions %s", buffer));
	return qtrue;
}

qboolean Script_Permission_f(itemDef_t *item, char **args) {
	 char ui_mpermusername[MAX_QPATH];
	 trap_Cvar_VariableStringBuffer("ui_mpermusername", ui_mpermusername, sizeof(ui_mpermusername) );
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mpermission %s %s", ui_mpermusername, uiInfo.PermList[uiInfo.PermIndex]));
	return qtrue;
}

qboolean Script_Login_f(itemDef_t *item, char **args) {
	 char ui_adminusername[MAX_QPATH];
	 char ui_adminpassword[MAX_QPATH];
	 trap_Cvar_VariableStringBuffer("ui_adminusername", ui_adminusername, sizeof(ui_adminusername) );
	 trap_Cvar_VariableStringBuffer("ui_adminpassword", ui_adminpassword, sizeof(ui_adminpassword) );
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mlogin %s %s\n", ui_adminusername, ui_adminpassword));
	return qtrue;
}

qboolean Script_Forbid_f(itemDef_t *item, char **args) {
	 char ui_mpermusername[MAX_QPATH];
	 trap_Cvar_VariableStringBuffer("ui_mpermusername", ui_mpermusername, sizeof(ui_mpermusername) );
     trap_Cmd_ExecuteText(EXEC_APPEND, va("mforbid %s %s", ui_mpermusername, uiInfo.PermList[uiInfo.PermIndex]));
	return qtrue;
}

static qboolean Script_LastMenu_f(char **args) {
	 char ui_menuname[MAX_QPATH];
	 char ui_itemname[MAX_QPATH];
	menuDef_t *menu;
	itemDef_t *item;	

	trap_Cvar_VariableStringBuffer("ui_menuname", ui_menuname, sizeof(ui_menuname) );
	trap_Cvar_VariableStringBuffer("ui_itemname", ui_itemname, sizeof(ui_itemname) );

	Menus_ActivateByName(ui_menuname);
	 	menu = Menus_FindByName(ui_menuname);
	...
	item = Menu_FindItemByName(menu, ui_itemname);
...
static qboolean Script_GetLastMenu_f(char **args) {
	 char ui_menuname[MAX_QPATH];
//	 char ui_itemname[MAX_QPATH];
	 char buffer[33];
	menuDef_t *menu;
...
//	 	menu = Menus_FindByName(ui_menuname);
	...
	itoa((int)menu,buffer,10);
//	if (buffer == "1077851908")
	if	( trap_Cvar_VariableValue( buffer ) == 1077851908 )
	{
	trap_Cvar_Set("ui_menuname", "ingame_makermod");
	}
	else if (buffer == "1077853240")
	{
	trap_Cvar_Set("ui_menuname", "ingame_makermod_emotes");
	}
...
#ifndef CGAME
	//SPINMOD REMOVED
  {"mGetLastMenu", &Script_GetLastMenu_f},	//Makermod Plugin v2 test.
  {"mLastMenu", &Script_LastMenu_f},			//Makermod Plugin v2 test.
  {"mForbid", &Script_Forbid_f},				//Makermod Plugin
  {"mLogin", &Script_Login_f},					//Makermod Plugin
  {"mPermission", &Script_Permission_f},		//Makermod Plugin
  {"mPermissions", &Script_Permissions_f},		//Makermod Plugin
  {"Animate3", &Script_Animate3_f},				//Makermod Plugin
  {"Animate3B", &Script_Animate3B_f},			//Makermod Plugin
  {"Animate2", &Script_Animate2_f},				//Makermod Plugin
  {"Animate2B", &Script_Animate2B_f},			//Makermod Plugin
  {"Animate", &Script_Animate_f},				//Makermod Plugin
  {"AnimateB", &Script_AnimateB_f},				//Makermod Plugin
  {"mResetPass", &Script_ResetPass_f},			//Makermod Plugin
  {"mChangePass", &Script_ChangePass_f},		//Makermod Plugin
  {"mNewUser", &Script_NewUser_f},				//Makermod Plugin
  {"mUnban", &Script_Unban_f},					//Makermod Plugin
  {"mSaveObs", &Script_SaveObs_f},				//Makermod Plugin
  {"mLoadObs", &Script_LoadObs_f},				//Makermod Plugin
  {"mScale", &Script_Scale_f},					//Makermod Plugin
  {"mPain", &Script_Pain_f},					//Makermod Plugin
  {"mBreakAble", &Script_BreakAble_f},			//Makermod Plugin
  {"mBreakAbleAll", &Script_BreakAbleAll_f},	//Makermod Plugin
  {"mForward1", &Script_Forward1_f},			//Makermod Plugin
  {"mBack1", &Script_Back1_f},					//Makermod Plugin
  {"mPlaceFx", &Script_PlaceFx_f},				//Makermod Plugin
  {"mRotateUp", &Script_RotateUp_f},			//Makermod Plugin
  {"mRotateDwn", &Script_RotateDwn_f},			//Makermod Plugin
  {"mRotate1", &Script_Rotate1_f},				//Makermod Plugin
  {"mRotate2", &Script_Rotate2_f},				//Makermod Plugin
  {"mRotate3", &Script_Rotate3_f},				//Makermod Plugin
  {"mRotate4", &Script_Rotate4_f},				//Makermod Plugin
  {"mRotate5", &Script_Rotate5_f},				//Makermod Plugin
  {"mRotate6", &Script_Rotate6_f},				//Makermod Plugin
  {"mRotate7", &Script_Rotate7_f},				//Makermod Plugin
  {"mRotate8", &Script_Rotate8_f},				//Makermod Plugin
  {"mMoveAllUp", &Script_MoveAllUp_f},			//Makermod Plugin
  {"mMoveAllDwn", &Script_MoveAllDwn_f},		//Makermod Plugin
  {"mMoveAll1", &Script_MoveAll1_f},			//Makermod Plugin
  {"mMoveAll2", &Script_MoveAll2_f},			//Makermod Plugin
  {"mMoveAll3", &Script_MoveAll3_f},			//Makermod Plugin
  {"mMoveAll4", &Script_MoveAll4_f},			//Makermod Plugin
  {"mMoveAll5", &Script_MoveAll5_f},			//Makermod Plugin
  {"mMoveAll6", &Script_MoveAll6_f},			//Makermod Plugin
  {"mMoveAll7", &Script_MoveAll7_f},			//Makermod Plugin
  {"mMoveAll8", &Script_MoveAll8_f},			//Makermod Plugin
  {"mMoveUp", &Script_MoveUp_f},				//Makermod Plugin
  {"mMoveDwn", &Script_MoveDwn_f},				//Makermod Plugin
  {"mMove1", &Script_Move1_f},					//Makermod Plugin
  {"mMove2", &Script_Move2_f},					//Makermod Plugin
  {"mMove3", &Script_Move3_f},					//Makermod Plugin
  {"mMove4", &Script_Move4_f},					//Makermod Plugin
  {"mMove5", &Script_Move5_f},					//Makermod Plugin
  {"mMove6", &Script_Move6_f},					//Makermod Plugin
  {"mMove7", &Script_Move7_f},					//Makermod Plugin
  {"mMove8", &Script_Move8_f},					//Makermod Plugin
  {"mArm", &Script_Arm_f},						//Makermod Plugin
  {"mSelect", &Script_Select_f},				//Makermod Plugin
  {"mTele", &Script_Tele_f},					//Makermod Plugin
  {"mScaleT", &Script_ScaleT_f},				//Makermod Plugin
  {"mListObs", &Script_ListObs_f},				//Makermod Plugin 
  {"mScaleMe", &Script_ScaleMe_f},				//Makermod Plugin 
#endif
...
//mp only
	...
	{"voicechat",		-1,					-1,		-1,	-1},
	{"ui_openmenu ingame_makermod",		-1,					-1,		-1,	-1},
	{"ui_openmenu ingame_makermod_testing2",	-1, 				-1,		-1,	-1},
...
//	{"sliderstop",		ItemParse_sliderstop,		NULL	},
