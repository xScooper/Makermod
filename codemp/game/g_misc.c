...
//	vec3_t originCorrection;
	if (player->s.eType == ET_NPC)
	{
		isNPC = qtrue;
	}

	if (player->client->sess.teleTimer > level.time)
		return;

/*	VectorCopy(origin, originCorrection);
	G_PlayerOrigin(player, &originCorrection);
	VectorCopy(originCorrection, origin);*/
	...
	player->client->sess.teleTimer = level.time + g_teleDelay.integer;
}
...
void SP_misc_model( gentity_t *ent ) {
	...
	trap_LinkEntity (ent);
	/*trap_SetConfigstring( 298, "models/map_objects/korriban/statue_sith_lord.md3");
	trap_SetBrushModel(ent, ent->model);
	G_Printf("TEEEEEEEEEEST");
	G_Printf("TEEEEEEEEEEST");
	G_Printf("TEEEEEEEEEEST");
	G_Printf("TEEEEEEEEEEST");
	G_Printf("TEEEEEEEEEEST");
...
void SP_misc_ammo_floor_unit(gentity_t *ent)
{
	...
	if ( ent->creator == 0 ) // only drop to ground if we're not a makermod ob
	{
		VectorSet( dest, ent->s.origin[0], ent->s.origin[1], ent->s.origin[2] - 4096 );
		trap_Trace( &tr, ent->s.origin, ent->r.mins, ent->r.maxs, dest, ent->s.number, MASK_SOLID );
		if ( tr.startsolid )
	...
	AddUse( ent, ammo_generic_power_converter_use );
...
void SP_misc_shield_floor_unit( gentity_t *ent )
{
	...
	AddUse( ent, shield_power_converter_use );
...
void SP_misc_model_ammo_power_converter( gentity_t *ent )
{
	...
	G_SpawnInt("nodrain", "0", &ent->genericValue12);
	AddUse( ent, ammo_power_converter_use );
...
void SP_misc_model_health_power_converter( gentity_t *ent )
{
	...
	AddUse( ent, health_power_converter_use );
...
void fx_runner_think( gentity_t *ent )
{
	...
	if ( ent->creator == 0 )
	{
		// this isn't a Maker Mod object, so mplace think won't be doing the moving.
...
#ifdef MM_THROW2
	if(ent->thrown)
		ent->fxthink = level.time + ent->delay + random() * ent->random;
	else ent->nextthink = level.time + ent->delay + random() * ent->random;
#endif
#ifndef MM_THROW2
	ent->nextthink = level.time + ent->delay + random() * ent->random;
#endif

	if ( ent->spawnflags & 4 && !ent->s.pos.trType == TR_INTERPOLATE ) // don't damage if the ob is grabbed
	{
		if(CanExistHere(ent->r.currentOrigin,ET_FX))
		{
			if (ent->creator)
			{
				attacker = &g_entities[ent->creator-1];
				G_RadiusDamage( ent->r.currentOrigin, attacker, ent->splashDamage, ent->splashRadius, ent, ent, MOD_CRUSH );
			}
...
{ 
	G_EffectIndex("*snow");
//	G_EffectIndex("*fog");
	G_EffectIndex("constantwind (0 0 100)");
}
...
void SP_target_screenshake(gentity_t *ent)
{
	...
	AddUse( ent, Use_Target_Screenshake );
}
...
void SP_target_escapetrig(gentity_t *ent)
{
	...
	AddUse( ent, Use_Target_Escapetrig );
}
...
void SP_misc_weapon_shooter( gentity_t *self )
{
    ...
    AddUse( self, misc_weapon_shooter_use );
