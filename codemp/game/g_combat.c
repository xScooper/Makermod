...
void player_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath ) {
	gentity_t	*ent;
	gentity_t	*creator = &g_entities[self->creator-1];
	int			anim;
	int			contents;
	int			killer;
	int			i = 0;
	...
	// Used by makermod respawners
	if ( self->s.eType == ET_NPC && self->closetarget )
	{
		G_Printf("firing closetarget %s", self->closetarget );
		G_UseTargets2( self, self, self->closetarget );
	}
...
#ifdef RANKINGMOD
	// Tell our score-keeping DLL about the death.
	if ( (g_gametype.integer == GT_DUEL) && level.numPlayingClients >= 2 && self->s.eType == ET_PLAYER && !self->NPC)
	{
		if ( attacker && attacker->client && attacker->client->NPC_class == CLASS_NONE &&
			                           self->client && self->client->NPC_class == CLASS_NONE)
			PlayerKilled( self - g_entities, attacker - g_entities, attacker->health, 
															attacker->client->pers.netname );
		else 
			PlayerDied( self - g_entities );
...
void G_Damage( gentity_t *targ, gentity_t *inflictor, gentity_t *attacker,
	...
	//Prevent a terrible loop :o -Scooper:
	/*if (targ->client && !(Q_stricmp( targ->classname, "player" ))) {
		if (!targ->client->ps.stats[STAT_HEALTH])
			return;
	}*/
	if (targ->health < 1 && (Q_stricmp( targ->classname, "bodyque" )))
		return;
...
qboolean G_RadiusDamage ( vec3_t origin, gentity_t *attacker, float damage, float radius,
	...
	*/
	...
	for ( e = 0 ; e < numListedEntities ; e++ ) {
		...
		if (!ent->takedamage)
			continue;
		if (ent->client) //mpain doesn't affect duels. -Scooper
		{
