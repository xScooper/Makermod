...
void PmoveSingle (pmove_t *pmove) {
	...
	else if ( pm->ps->torsoAnim == BOTH_SIT1 || pm->ps->torsoAnim == BOTH_SIT2 || pm->ps->torsoAnim == BOTH_SIT3 || pm->ps->torsoAnim == BOTH_SIT4 ||
		      pm->ps->torsoAnim == BOTH_SLEEP1 || pm->ps->torsoAnim == BOTH_STAND4 || pm->ps->torsoAnim == BOTH_STAND_TO_KNEEL )
	{
		if ( (pm->cmd.buttons&BUTTON_ATTACK)
			|| (pm->cmd.buttons&BUTTON_ALT_ATTACK)
			...
			|| pm->cmd.upmove )
		{//stop the anim
			if ( pm->ps->torsoAnim == BOTH_SIT1 )
			{
				PM_SetAnim( SETANIM_BOTH, BOTH_MEDITATE_END, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD, 0 );
			}
			else if ( pm->ps->torsoAnim == BOTH_SIT2 )
			{
				PM_SetAnim( SETANIM_BOTH, BOTH_SIT2TOSTAND5, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD, 0 );
			}
			else if ( pm->ps->torsoAnim == BOTH_SIT3 )
			{
				PM_SetAnim( SETANIM_BOTH, BOTH_SIT3TOSTAND5, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD, 0 );
			}
			else if ( pm->ps->torsoAnim == BOTH_SIT4 )
			{
				PM_SetAnim( SETANIM_BOTH, BOTH_SIT3TOSTAND5, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD, 0 );
			}
			else if ( pm->ps->torsoAnim == BOTH_SLEEP1 )
			{
				PM_SetAnim( SETANIM_BOTH, BOTH_SLEEP1GETUP2, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD, 0 );
			}
			else if ( pm->ps->torsoAnim == BOTH_STAND4 )
			{
				PM_SetAnim( SETANIM_BOTH, BOTH_STAND4TO2, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD, 0 );
			}
			else if ( pm->ps->torsoAnim == BOTH_STAND_TO_KNEEL )
			{
				PM_SetAnim( SETANIM_BOTH, BOTH_KNEEL_TO_STAND, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD, 0 );
			}
			else
			{
				pm->ps->legsTimer = pm->ps->torsoTimer = 0;
