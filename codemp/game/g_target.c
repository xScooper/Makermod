...
void SP_target_remove_powerups( gentity_t *ent ) {
	AddUse( ent, Use_target_remove_powerups );
}
...
if (0)
{
	...
	G_ActivateBehavior(ent,BSET_USE);
	if ( ( ent->spawnflags & 4 ) ||  ent->creator != 0 ) //makermod obs are always private 
	{//private, to one client only
		...
		{//make sure there's a valid client ent to send it to
			if (ent->message[0] == '@' && ent->message[1] != '@')
			{
				...
				activator->client->pers.lastMessageTime = level.time;
			}
			else
			{
...
void target_teleporter_use( gentity_t *self, gentity_t *other, gentity_t *activator ) {
	...
	{
		TeleportPlayer( activator, self->pos3, activator->s.angles );
		return;
	}
...
void SP_target_teleporter( gentity_t *self ) {
	if ( self->creator == 0 && !self->targetname )
		G_Printf("untargeted %s at %s\n", self->classname, vtos(self->s.origin));

	AddUse( self, target_teleporter_use );
}
...
void target_relay_use (gentity_t *self, gentity_t *other, gentity_t *activator) {
	...
	{//never use again
		if ( ranscript )
		{//crap, can't remove!
			RemoveUse( self, target_relay_use );
		}
		else
		{//remove
...
void target_kill_use( gentity_t *self, gentity_t *other, gentity_t *activator ) {
	G_ActivateBehavior(self,BSET_USE);
	G_Damage ( activator, NULL, NULL, NULL, NULL, 100000, DAMAGE_NO_PROTECTION, MOD_UNKNOWN); //MOD_TELEFRAG);
}

void SP_target_kill( gentity_t *self ) {
	AddUse( self, target_kill_use );
}
...
void SP_target_counter (gentity_t *self)
{
	...
	AddUse( self, target_counter_use );
}
...
void target_random_use(gentity_t *self, gentity_t *other, gentity_t *activator)
{
	...
	if(self->spawnflags & 1)
	{
		RemoveUse( self, target_random_use );
	}
...
void scriptrunner_run (gentity_t *self)
{
	...
	*/
	...
	if ( self->count != -1 )
	{
		if ( self->count <= 0 )
		{
			RemoveUse( self, target_scriptrunner_use );
...
void SP_target_scriptrunner( gentity_t *self )
{
	...
	*/
	...
	G_SetOrigin( self, self->s.origin );
	AddUse( self, target_scriptrunner_use );
}
...
{
	G_SetOrigin( self, self->s.origin );
	AddUse( self, target_activate_use );
}
...
{
	G_SetOrigin( self, self->s.origin );
	AddUse( self, target_deactivate_use );
}
...
void SP_target_level_change( gentity_t *self )
{
	...
	G_SetOrigin( self, self->s.origin );
	AddUse( self, target_level_change_use );
}
...
void SP_target_play_music( gentity_t *self )
{
	...
	AddUse( self, target_play_music_use );
}
