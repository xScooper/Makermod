...
#include "../game/g_local.h"
	...
	void CrashHandler(int signal, siginfo_t *siginfo, ucontext_t *ctx) {
		...
		if(segvloop < 2) {
			char	rawmapname[MAX_PATH];
			trap_Cvar_VariableStringBuffer("mapname", rawmapname, sizeof(rawmapname));
			G_LogPrintf("-8<------- Crash Information ------->8-\n");
			G_LogPrintf("    Please forward to the NDA team.    \n");
			G_LogPrintf("---------------------------------------\n");
...
#pragma pack (pop)
	...
	LONG CALLBACK win32_exception_handler(LPEXCEPTION_POINTERS e) {
		...
		G_LogPrintf("-8<------- Crash Information ------->8-\n");
		G_LogPrintf("    Please forward to Scooper \n");
		G_LogPrintf("---------------------------------------\n");
