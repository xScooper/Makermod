...
qboolean G_MoverPush( gentity_t *pusher, vec3_t move, vec3_t amove, gentity_t **obstacle ) {
	...
	*obstacle = NULL;
	...
	for ( e = 0 ; e < listedEntities ; e++ ) {
		...
		//Maker Mod - Keep user-created objects moving, even if they bump into something
		if ( check->creator > 0 )
			continue;
...
//	gentity_t	*other = ent->enemy;
	...
	if ( ent->moverState == MOVER_POS1 ) 
	{
		...
		if (ent->creator && activator->client )
		{
			if (ent->passworded && (!(strcmp(ent->objectPassword, activator->client->sess.objectPassword) == 0) ) )
			{
				if ((activator->client->sess.touchTimer-1 < level.time))
				{
					MM_SendMessage( activator-g_entities, va("print \"This object is passworded.\n\""));
					activator->client->sess.touchTimer = level.time + 3500; // 3.5 secs to make it look better ingame. - Scooper
					level.time++;
				}
				return;
			}
...
void Use_BinaryMover( gentity_t *ent, gentity_t *other, gentity_t *activator ) 
{
	...
	if( !CanExistHere( ent->r.currentOrigin, ENT_FX))
		return;
	...
	//Scooper: This seems useless, and it's also causing some troubles.
//	ent->enemy = other;
	ent->activator = activator;
	if(ent->delay)
	{
...
void InitMover( gentity_t *ent ) 
{
	...
	if ( ent->model2 ) 
	{
		...
		{
			// we're applying mover functionality for the first time
			ent->s.modelindex2 = G_ModelIndex( ent->model2 );
		}
	}
	//Makermod - Delta:
	VectorSubtract(ent->pos2, ent->pos1, ent->pos1to2);
...
/*	//ColoUr for finer control of colour with makermod.
	lightSet = G_SpawnFloat( "light", "100", &light );
	colorSet = G_SpawnVector( "colour", "255 255 255", color );
	if ( lightSet || colorSet ) {
		int		r, g, b, i;
...
void SP_func_door (gentity_t *ent) 
{
	...
	if ( !(ent->flags&FL_TEAMSLAVE) ) 
	{
		...
		trig = 0;
		temp = g_entities;
		while ( (temp = G_Find( temp, FOFS( classname ), "trigger_door" )) != NULL )
		{
			if ( temp->parent == ent )
			{
				trig = temp;
...
void Touch_Plat( gentity_t *ent, gentity_t *other, trace_t *trace ) {
	...
	if (ent->creator && other->client )
		{
			if (ent->passworded && (!(strcmp(ent->objectPassword, other->client->sess.objectPassword) == 0) ) )
			{
				return;
			}
		}
...
//	ent->soundPos1 = ent->soundPos2 = G_SoundIndex("sound/movers/plats/pt1_end.wav");
	...
	if ( !ent->targetname || ent->creator == 0 ) // creator is 0 if not a makermod ob
	{
...
void SP_func_static( gentity_t *ent ) 
{
	...
	// i	
	//if ( ent->creator == 0 || ent->use == 0 || ent->use == Use_BinaryMover )

	AddUse( ent, func_static_use );
...
void SP_func_rotating (gentity_t *ent) {
	...
	{
		tempModelScale = ent->s.iModelScale;
		G_SpawnInt("model2scale", va("%i",tempModelScale), &ent->s.iModelScale);
	}
...
void G_MiscModelExplosion( vec3_t mins, vec3_t maxs, int size, material_t chunkType )
{
	...
	// Don't cause the server to drop if someone blows up their entire breakable house.
						...
						float speed, int numChunks, material_t chunkType, int customChunk, float baseScale )
{
	...
	te = G_TempEntity( origin, EV_DEBRIS );
...
static void InitBBrush ( gentity_t *ent ) 
{
	...
	// if the "model2" key is set, use a seperate model
	// for drawing, but clip against the brushes
	if ( ent->model2 && ent->model2[0] && ent->s.eType == 0 ) 
	{
		ent->s.modelindex2 = G_ModelIndex( ent->model2 );
	}
...
{//Toggle on and off
	...
	{//ALWAYS_ON
		...
		RemoveUse( self, func_usable_use );
