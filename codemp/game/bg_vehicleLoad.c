...
//Makermod: Check vehicle without loading it.
...
int VEH_CheckVehicle( const char *vehicleName )
{//load up specified vehicle and save in array: g_vehicleInfo
	const char	*token;
	//we'll assume that no parm name is longer than 128
	char		parmName[128] = { 0 };
	...
	while ( 1 ) 
	{
		...
		else if ( Q_stricmp( "model", parmName ) == 0 )
		{
...
#ifndef CGAME
#ifndef _USRDLL
			if( !Q_stricmp(npcTypesArray[i].NPCTypes, model))
#endif
#endif
				{
					return (VEH_LoadVehicle(vehicleName));
				}
		}
	}
	return VEHICLE_NONE;
...
//Makermod: Don't add.
int VEH_VehicleIndexForNameDontADD( const char *vehicleName )
{
	int v;
	if ( !vehicleName || !vehicleName[0] )
	{
	...
	v = VEH_CheckVehicle( vehicleName );
	if(v != VEHICLE_NONE)
		return v;
...
{
	char *vehName = &modelname[1];
	int vIndex = VEH_VehicleIndexForNameDontADD(vehName);
	assert(modelname[0] == '$');
