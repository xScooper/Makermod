...
void SpectatorThink( gentity_t *ent, usercmd_t *ucmd ) {
	...
	if ( client->sess.spectatorState != SPECTATOR_FOLLOW ) {
		...
		pm.ps = &client->ps;
		pm.cmd = *ucmd;
		pm.tracemask = MASK_PLAYERSOLID & ~CONTENTS_BODY;//|CONTENTS_BODY// & ~CONTENTS_SOLID;	// spectators can fly through bodies
...
qboolean ClientInactivityTimer( gclient_t *client ) {
	if ( ! g_inactivity.integer ) {
		...
		if ( level.time > client->inactivityTime - 10000 && !client->inactivityWarning ) {
			...
			client->pers.lastMessageTime = level.time;
		}
	}
	return qtrue;
}
...
void G_SetTauntAnim( gentity_t *ent, int taunt )
{
	...
	// hack, if we've frozen in one of Maker Mod's emote's 
	// unfreeze now
...
void ClientThink_real( gentity_t *ent ) {
	...
	// Display Makermod automated messages
	if ( !isNPC && level.time - client->pers.lastMessageTime > 2700 ) //2.7secs between screen prints
	{
		clock_t now; 
		...
		if ( client->pers.firstEnterTime <= now )
		{
			time = client->sess.totalTime + max( 0, now - client->pers.firstEnterTime)/CLOCKS_PER_SEC;
		}
		else
		{
			// handle one loop around of the process clock
			time = client->sess.totalTime + max( 0, now + 2^(8 * sizeof(clock_t)) - client->pers.firstEnterTime )/CLOCKS_PER_SEC;
		}

		#ifndef MM_RELEASE
		if(level.time - client->pers.enterTime < 5000 && client->sess.sessionTeam != TEAM_SPECTATOR)
		{
//			trap_SendServerCommand( ent - g_entities, va("chat \"^7Welcome to the official Makermod Test server.\""));
//			trap_SendServerCommand( ent - g_entities, va("chat \"^7Please report any bugs you discover in makermod on our forum: www.makermod.net\""));
//			trap_SendServerCommand( ent - g_entities, va("chat \"^1Notice: ^7I've made some changes to the loading system. Please report any broken saves.\""));
		}
		#endif
		// [RemapObject]
		// Makermod
		if(playerClients[ent - g_entities].numMessages == 1 && playerClients[ent - g_entities].messageTime < level.time )
		{
			if(!playerClients[ent - g_entities].hasClient || playerClients[ent - g_entities].version < MMS_VERSION)
			{
				trap_SendServerCommand( ent - g_entities, va("chat \"^7Welcome, there is a client plugin update available.\""));
				trap_SendServerCommand( ent - g_entities, va("chat \"^7You can download it at www.makermod.net, under the release category of the forum.\""));
				trap_SendServerCommand( ent - g_entities, va("chat \"^7For more information do /minfo update.\""));
			}
			playerClients[ent - g_entities].numMessages++;
		}
		// [RemapObject] end
		if ( level.time < level.announceStop )
		{
			trap_SendServerCommand( ent - g_entities, va("cp \"%s\n\"", level.announcement) );
		}
		else if ( (level.time - client->pers.enterTime < 10000 && client->sess.sessionTeam != TEAM_SPECTATOR && g_gametype.integer != GT_DUEL) ||
			   (level.time - client->pers.enterTime < 1000 && client->sess.sessionTeam != TEAM_SPECTATOR && g_gametype.integer == GT_DUEL) )
		{
			trap_SendServerCommand( ent - g_entities, va("cp \"%s\n\"", g_JoinMOTD.string) );
		}
		else if ( (time / 1800 > 1 && time % 900 < 5) )
		{
			int hours = time/3600;
			//G_Printf( "%i %i %i,", client->sess.totalTime, (int)client->pers.firstEnterTime, (int)now );

			
			if ( hours == 1 )
				trap_SendServerCommand( ent - g_entities, va("cp \"You've been playing for %i hour %i mins\n\"", hours, (time/60)%60  ) );
			else if ( hours > 1 )
				trap_SendServerCommand( ent - g_entities, va("cp \"You've been playing for %i hours %i mins\n\"", hours, (time/60)%60  ) );
			else
				trap_SendServerCommand( ent - g_entities, va("cp \"You've been playing for %i minutes\n\"", time/60  ) );
		}

		if ( level.time - client->pers.lastMessageTime < 180000 || client->pers.lastMessageTime == 0 )
			client->pers.lastMessageTime = level.time;

		if ( level.time > level.announceStop )
			level.announceStop = 0;
	}

	//Anti Chat Flood system, -Scooper
	if( g_antiChatFlood.integer) {
		if (ent->chatSpam > 0 && ent->chatChanged + 1000 < level.time)
			ent->chatSpam = 0;
	}
	//Anti Userinfo Flood system, -Scooper
	if( g_antiUserinfoFlood.integer) {
		if (ent->userinfoSpam > 0 && ent->userinfoChanged + 1000 < level.time)
			ent->userinfoSpam = 0;
	}

/*	if (ent->chatSpam > 5 && ent->chatSpamReset < level.time) 
		ent->chatSpamReset = level.time + 2000;*/
...
//		G_Printf("serverTime >>>>>\n" );
	...
	if ( client->godModFlags & GMOD_EMPOWERED )
	{
		client->ps.eFlags |= EF_BODYPUSH;
	}
	else if (client->pushEffectTime > level.time)
	{
		client->ps.eFlags |= EF_BODYPUSH;
	}
	...
	if( client && !isNPC)
	{
		if( ucmd->buttons & BUTTON_THROW && !(client->sess.makerModFlags & GMOD_THROWING) )
		{
			client->sess.makerModFlags |= GMOD_THROWING;
			StartThrow(ent);
		}
		else if ( !(ucmd->buttons & BUTTON_THROW) && (client->sess.makerModFlags & GMOD_THROWING) )
		{
			client->sess.makerModFlags &= ~GMOD_THROWING;
			EndThrow(ent);
		}
	...
	if ( client && !isNPC && ucmd->upmove == 0 && !(ucmd->buttons & BUTTON_ATTACK) && 
		( client->ps.torsoAnim == BOTH_SIT1 || client->ps.torsoAnim == BOTH_SIT2 || client->ps.torsoAnim == BOTH_SIT3 || client->ps.torsoAnim == BOTH_SIT4 || client->ps.torsoAnim == BOTH_SLEEP1 || client->ps.torsoAnim == BOTH_STAND4 || client->ps.torsoAnim == BOTH_STAND_TO_KNEEL) )
	{
		//client->ps.eFlags |= EF2_HELD_BY_MONSTER;
		//ucmd->forwardmove = 0;
		//ucmd->rightmove = 0;
		//ucmd->upmove = 0;
		ucmd->buttons = 0;
	...
	//{
	//	// make sure we can get up and/or change animations
	//	client->ps.pm_type = PM_NORMAL;
	//}
	...
	if (ent->client->ps.duelInProgress)
	{
		...
		if (ent->client->ps.duelTime < level.time)
		{
			...
			{	
				...
				ent->client->ps.stats[STAT_ARMOR] = ent->client->ps.stats[STAT_MAX_HEALTH] * 1; //Duel Shield changed from 0.25 to 1 -Scooper
				...
				&& duelAgainst->client->ps.duelTime)
			{
				...
				G_Unempower( duelAgainst );

				ent->flags &= ~FL_GODMODE;
				duelAgainst->client->ps.stats[STAT_ARMOR] = client->ps.stats[STAT_MAX_HEALTH] * 1; //Duel Shield changed from 0.25 to 1 -Scooper
				duelAgainst->client->ps.stats[STAT_HEALTH] = duelAgainst->health = duelAgainst->client->ps.stats[STAT_MAX_HEALTH];
				duelAgainst->client->ps.stats[STAT_HOLDABLE_ITEMS] &= ~(1 << HI_JETPACK);
		...
		else if (duelAgainst->health < 1 || duelAgainst->client->ps.stats[STAT_HEALTH] < 1)
		{
			...
			{
				//Save the duel health for now, so it can be broadcasted.
				ent->client->duelHealth = ent->client->ps.stats[STAT_HEALTH];
			...
			*/
			...
			if (ent->health > 0 && ent->client->ps.stats[STAT_HEALTH] > 0)
			{
				trap_SendServerCommand( -1, va("cp \"%s %s %s!\n ^7With ^1%i^7/^2%i^7\n\"", ent->client->pers.netname, G_GetStringEdString("MP_SVGAME", "PLDUELWINNER"), duelAgainst->client->pers.netname, ent->client->duelHealth, ent->client->ps.stats[STAT_ARMOR]) );
				trap_SendServerCommand( -1, va("print \"%s %s %s^7 in a duel, with ^1%i^7/^2%i^7\n\"", ent->client->pers.netname, G_GetStringEdString("MP_SVGAME", "PLDUELWINNER"), duelAgainst->client->pers.netname, ent->client->duelHealth, ent->client->ps.stats[STAT_ARMOR]) );
				G_LogScreenPrintAll();
			}
			else
...
/*	else if ( client->noclip ) {
		pm.tracemask = CONTENTS_BODY;//CONTENTS_NOCLIP;
	}*/
	else {
		/*if ( client->noclip )	// [Makermod noclip]
			pm.tracemask = CONTENTS_NOCLIP;
	...
	*/
		...
		(pm.cmd.generic_cmd != ent->client->lastGenCmd || ent->client->lastGenCmdTime < level.time))
	{
		...
		case GENCMD_USE_SEEKER:
			...
			{
				if ( level.entCount > MAX_GENTITIES - (g_objectMargin.integer/2) )
				{
					if ( ent )
						trap_SendServerCommand( ent-g_entities, va("print \"ERROR: Unable to use item. There are already too many objects.\n\""));
					break;
				}
				ItemUse_Seeker(ent);
...
//		TryUse( ent );
	...
	{
		ent->client->ps.useDelay = level.time + 400; // pre-makermod, this was 100
		TryUse(ent);
	}
...
//	G_VehicleAttachDroidUnit( ent );
	...
	if ( (client->godModFlags & GMOD_DRAWING) && level.time > client->nextDrawTime )
	{
		trace_t		trace;
		vec3_t		src, dest, vf;
		vec3_t		viewspot, endpoint;
		...
		trap_Trace( &trace, src, vec3_origin, vec3_origin, dest, ent->s.number, MASK_OPAQUE|CONTENTS_SOLID|CONTENTS_BODY|CONTENTS_TRIGGER);
		...
		if ( endpoint )
		{	
			if ( client->godModFlags & GMOD_START_DRAWING )
				client->godModFlags &= ~GMOD_START_DRAWING;
			else
				G_TestLine( client->lastDrawSpot, endpoint, 0, 600000 );

			VectorCopy( endpoint, client->lastDrawSpot );

			client->nextDrawTime = level.time + 200;
		}
	}
...
// Maker Mod function to get around the client side clipping problem for 
// objects with larger x's than y's
// position - position of our entity
void SwapClippingBounds( vec3_t position )
{
	...
	for( i = MAX_CLIENTS ; i < level.num_entities ; i++ )
	{
		...
		if ( obj->creator == 0 || !(obj->inuse) || obj->NPC || obj->s.eType != ET_MOVER )
			continue;

		if ( obj->ymaxs == obj->r.maxs[0] && obj->ymins == obj->r.mins[0] )
			continue;

		if ( Distance(obj->r.currentOrigin, position) > 1000 )
			continue;
		
		tempMax = obj->r.maxs[0];
		tempMin = obj->r.mins[0];
		obj->r.maxs[0] = obj->ymaxs;
		obj->r.mins[0] = obj->ymins;
		obj->ymaxs = tempMax;
		obj->ymins = tempMin;
...
void ClientThink( int clientNum,usercmd_t *ucmd ) {
	...
	if ( clientNum < MAX_CLIENTS && !(ent->r.svFlags & SVF_BOT) && !g_synchronousClients.integer )
		SwapClippingBounds( position );
...
void G_RunClient( gentity_t *ent ) {
	...
	if ( g_entities - ent < MAX_CLIENTS && !(ent->r.svFlags & SVF_BOT) )
		SwapClippingBounds( position );
...
void SpectatorClientEndFrame( gentity_t *ent ) {
	...
	else if( !level.numPlayingClients && (ent->client->sess.spectatorClient == -1 || ent->client->sess.spectatorClient == -2) )
	{
		ent->client->sess.spectatorState = SPECTATOR_FREE;
		ClientBegin( ent->client - level.clients, qtrue );
	}
