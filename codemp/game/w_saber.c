...
// nmckenzie: SABER_DAMAGE_WALLS
		...
		//Maker Mod Poke Protection START

		oldHealth = victim->health;

		if ( self->client->ps.fd.saberAnimLevel == SS_STRONG && victim->health > 0 && victim->client ) 
		{
			oldHealth = victim->health;

			oldFlags = victim->flags;

			victim->flags |= FL_UNDYING;

			// FL_UNDYING only lets our health go as low as 1 but we want to be able 
			// to detect damage to a 1hp player. There's to much logic in G_Damage to 
			// do this any other way, unfortunately.
			if ( victim->health == 1 )
				victim->health++; 		
		}
		...
		if ( self->client->ps.fd.saberAnimLevel == SS_STRONG && victim->health > 0 && victim->client )
		{
			victim->flags = oldFlags;

			if ( self->client->dirChangeCount >= 0 && (oldHealth - victim->health > 0 
				   || (oldHealth == 1 && victim->health == 1)/*undamaged, it would be 2*/))
			{
#ifndef MM_RELEASE
				if ( (self->client->sess.permissions & PERMISSION_TESTING) ) //(!g_cheats.integer) &&
				{
					trap_SendServerCommand( self-g_entities, va("print \"^3Attempting Refund \"" ));
				}
#endif

				// Look for an empty or matching refund record
				for( j = 0 ; j < MAX_POKE_REFUNDS &&  self->client->pokeRefundClient[j] != -1 &&
										self->client->pokeRefundClient[j] != victimEntityNum[i] ; j++ ) 
										;

				if ( j < MAX_POKE_REFUNDS )
				{
#ifndef MM_RELEASE
					if ( (self->client->sess.permissions & PERMISSION_TESTING) ) //(!g_cheats.integer) &&
					{
						trap_SendServerCommand( self-g_entities, va("print \"Queuing Refund %ihp,ent%i; \"", oldHealth - victim->health, victimEntityNum[i] ));
					}
#endif

					self->client->pokeRefundClient[j] = victimEntityNum[i];
					self->client->pokeRefundAmount[j] += oldHealth - max( victim->health, 0 );

					if ( victim->health == 1 ) // G_Damage with FL_UNDYING leaves hp at 1 if we would have died.
					{
						self->client->pokeRefundDeath[j] = qtrue; // this reminds us that our victim would have died
						VectorCopy( dmgDir[i], self->client->lastHitDirection );
						VectorCopy( dmgSpot[i], self->client->lastHitSpot );
					}
				}
			}
		}

		// Remove the adjustment we made earlier to detect damage to a 1hp player with FL_UNDYING
		if ( self->client->ps.fd.saberAnimLevel == SS_STRONG && oldHealth == 1 && victim->health == 2 )
		{
			victim->health = 1;
		}

		//Maker Mod Poke Protection END
	}
}
...
static GAME_INLINE qboolean CheckSaberDamage(gentity_t *self, int rSaberNum, int rBladeNum, vec3_t saberStart, vec3_t saberEnd, qboolean doInterpolate, int trMask, qboolean extrapolate )
{
	...
	// Start modified for Maker Mod POKE Protecion
	if ( !SaberAttacking(self) || BG_SabersOff( &self->client->ps ) )
	{
		...
		//if ( (self->client->sess.permissions & PERMISSION_TESTING) ) //(!g_cheats.integer) &&
		//{
	//		trap_SendServerCommand( self-g_entities, va("print \"z \"", SHORT2ANGLE(self->client->pers.cmd.angles[YAW]), self->client->pers.cmd.angles[YAW], self->client->dirChangeCount ));
	//	}
		self->client->attackYawDirection = 0;
		self->client->godModFlags &= ~GMOD_POKING;

		for ( i = 0 ; i < MAX_POKE_REFUNDS && self->client->pokeRefundClient[i] != -1 ; i++ )
		{
			if ( self->client->pokeRefundDeath[i] == qtrue &&
				     g_entities[self->client->pokeRefundClient[i]].inuse && 
					    g_entities[self->client->pokeRefundClient[i]].client != 0 )
			{
				// Kill our victim, if we aren't 
				G_Damage( &g_entities[self->client->pokeRefundClient[i]], self, self,
							g_entities[self->client->pokeRefundClient[i]].client->lastHitDirection,
								g_entities[self->client->pokeRefundClient[i]].client->lastHitSpot, 
									9999, 0, MOD_SABER );
			}

			self->client->pokeRefundClient[i] = -1;
			self->client->pokeRefundAmount[i] = 0;
			self->client->pokeRefundDeath[i] = qfalse;
		}
	...
	// End modified for Maker Mod POKE PROTECTION
	...
	// Start Makermod Poke Protection
	if ( self->client->godModFlags & GMOD_POKING )
	{
		dmg = 0;	
	}
	else if ( dmg > 0 && self->client->ps.fd.saberAnimLevel == SS_STRONG )
	{	
		float yawRange = 0; //change in Yaw since last change in rotation direction.
		float deltaYaw; // change in yaw since last time we thought about it.

		self->client->attackSamples++;

		//if ( (self->client->sess.permissions & PERMISSION_TESTING) ) //(!g_cheats.integer) &&
		//{
			//trap_SendServerCommand( self-g_entities, va("print \"%f,%f,%i,%i; \"", self->r.currentAngles[YAW], SHORT2ANGLE(self->client->pers.cmd.angles[YAW]), self->client->pers.cmd.angles[YAW], self->client->dirChangeCount ));
		//}

		deltaYaw = AngleNormalize360(SHORT2ANGLE(self->client->pers.cmd.angles[YAW])) - AngleNormalize360(SHORT2ANGLE(self->client->lastAttackYaw));

		//else if ( (SHORT2ANGLE(self->client->pers.cmd.angles[YAW]) - SHORT2ANGLE(self->client->lastAttackYaw) >0) && //ANGLE2SHORT(0.1))  && 
		//	        (SHORT2ANGLE(self->client->pers.cmd.angles[YAW]) - SHORT2ANGLE(self->client->lastAttackYaw) < 180 ) )
		if ( deltaYaw > 0 && deltaYaw < 180 ) //ANGLE2SHORT(0.1))  && 
		   //     (SHORT2ANGLE(self->client->pers.cmd.angles[YAW]) - SHORT2ANGLE(self->client->lastAttackYaw) < 180 ) )



		//if ( (SHORT2ANGLE(self->client->ps.delta_angles[YAW]) > 0) )
		{
#ifndef MM_RELEASE
				if ( (self->client->sess.permissions & PERMISSION_TESTING) ) //(!g_cheats.integer) &&
				{
					trap_SendServerCommand( self-g_entities, va("print \"u \"", SHORT2ANGLE(self->client->pers.cmd.angles[YAW]), self->client->pers.cmd.angles[YAW], self->client->dirChangeCount ));
				}
#endif
			if ( self->client->attackYawDirection < 0 && level.time - self->client->lastYawDirChange < GMODPOKETIME )
			{
				self->client->dirChangeCount++;
				yawRange = SHORT2ANGLE(self->client->lastAttackYaw) - SHORT2ANGLE( self->client->attackExtreme );
				self->client->attackExtreme = self->client->lastAttackYaw;
			}

			if ( self->client->attackYawDirection == 0 )
			{
				self->client->dirChangeCount = 0;
				self->client->attackExtreme = 0;
				self->client->attackSamples = 0; 
			}
			else
				self->client->lastYawDirChange = level.time;

			if ( self->client->dirChangeCount > 1 )
			{
				if ( yawRange > 25 || yawRange < -25 )
					self->client->godModFlags |= GMOD_POKING;
				else
					self->client->dirChangeCount--;
			...
			self->client->attackYawDirection = 1; 
		}
		else if ( deltaYaw < 0. && deltaYaw > -180 )
		//else if (  self->client->pers.cmd.angles[YAW] - self->client->lastAttackYaw < 0 && self->client->pers.cmd.angles[YAW] - self->client->lastAttackYaw > -1*ANGLE2SHORT(180))
		//else if ( (SHORT2ANGLE(self->client->lastAttackYaw) - SHORT2ANGLE(self->client->pers.cmd.angles[YAW]) > 0)&& //ANGLE2SHORT(0.1)) &&
		///	        ( SHORT2ANGLE(self->client->lastAttackYaw) - SHORT2ANGLE(self->client->pers.cmd.angles[YAW]) < 180 )  )
		//if ( SHORT2angleself->client->ps.delta_angles[YAW] < 3 )	
		//else if ( (SHORT2ANGLE(self->client->ps.delta_angles[YAW]) < 0) )
		{
			//float yawRange; //change in Yaw since last change in rotation direction.

#ifndef MM_RELEASE
			if ( (self->client->sess.permissions & PERMISSION_TESTING) ) //(!g_cheats.integer) &&
			{
				trap_SendServerCommand( self-g_entities, va("print \"d \"", SHORT2ANGLE(self->client->pers.cmd.angles[YAW]), self->client->pers.cmd.angles[YAW], self->client->dirChangeCount ));
			}
#endif
		...
		{
			int i;
			// We've just detected some pokeage. Moan about it!
			//trap_SendServerCommand( self-g_entities, va("print \"^1Poke detected and neutralised\n\"" ));

			for( i = 0; i < MAX_POKE_REFUNDS && self->client->pokeRefundClient[i] != -1 ; i++ )
			{
				if ( g_entities[self->client->pokeRefundClient[i]].client != 0 )
				{
					//trap_SendServerCommand( self-g_entities, va("print \"^2%ihp^7 pre-detection damage refunded\n\"",self->client->pokeRefundAmount[i] ));
					g_entities[self->client->pokeRefundClient[i]].health += self->client->pokeRefundAmount[i];
					g_entities[self->client->pokeRefundClient[i]].client->ps.stats[STAT_HEALTH] = g_entities[self->client->pokeRefundClient[i]].health;
					self->client->pokeRefundDeath[i]= qfalse;
				}
			}
	...
	self->client->lastAttackYaw = self->client->pers.cmd.angles[YAW];
	// End Makermod Poke Protection
