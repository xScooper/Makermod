...
#define	GAMEVERSION	MM_GAMEVERSION//"Makermod"
#ifdef RANKINGMOD
#define MODVERSION  "1.0beta+Scoring1.0"
#else
#define MODVERSION  MM_MODVERSION//"1.3beta"
#endif
...
// IF YOU MODIFY THESE, ALSO MODIFY THE PERMISSION CODES ARRAY IN g_cmds.c

#define MAX_USER_PERMISSION_CODE 30
...
#define GMOD_EMPOWERED		0x00000010
#define GMOD_POKING			0x00000100  //client has tried to do a poke
#define GMOD_GRABBING		0x00001000  //client is automatically grabbing placed obs + fx's
#define GMOD_DRAWING		0x00010000  //client is drawing on the map
#define GMOD_START_DRAWING	0x00100000
#define GMOD_REFUSING		0x01000000	//client is refusing giveother. //Scoop
#ifdef MM_THROW2
#define GMOD_THROWING		0x10000000	// client is throwing something.
#endif
#define GMOD_SILENT			0x00000020	// Silent mode, stops client from receiving print messages from commands.


// Maker Mod Values

#define GMODUNDEFINED 20000000
#define GMODPOKETIME 1000 // milliseconds

// Maker Mod Entity Descriptions
#define ENT_NPC		0x000001
#define ENT_VEHICLE	0x000010
#define ENT_OBJECT  0x000100
#define ENT_FX		0x000100
#define ENT_GUN		0x000100


#define OFFSET_TO_FEET 24
...
//typedef struct { void (*use)(gentity_t *self, gentity_t *other, gentity_t *activator); void* next; } extra_use_t;

#define MAX_EXTRA_USE 10

typedef void (*use_t)(gentity_t *self, gentity_t *other, gentity_t *activator);
...
//Only used by NPC_spawners
	...
	vec3_t		pos1to2;		//Makermod - Pos1 Pos2 delta, (difference)
...
//Health and damage fields
	...
	// for Maker Mod
	int passworded; //If the object is passworded.
	int creator; // clientNum that created this item
	char objectPassword[15 + 1];
	float ymaxs;
	float ymins;
	int grabRadius; 
	char* spawnFields;  // spawn-fields that we want to savZe in our Maker Mod file
	void (*oldThink)(gentity_t *self);
	use_t extraUse[MAX_EXTRA_USE] ;
	qboolean grabbing; //If object is being grabbed at the moment.
	int chatSpam; //Holds amount of messages lately sent
	int chatChanged; //The time he last chatted.
	int userinfoSpam; //Holds amount of userinfo changes.
	int userinfoChanged; //The time he last changed userinfo.
#ifdef MM_THROW2
	vec3_t oldOrigin;
	int fxthink;
	qboolean thrown;	// Being thrown or not.
#endif
	char *objName;		// Pointer to objects name.
	int	*objNameUse;	// How many of it is in use.
...
#ifdef RANKINGMOD
	char		playerUsername[MAX_USERNAME_LENGTH];
#endif
} clientSession_t;
...
typedef struct {
	...
	int			lastMessageTime;	// New For Makermod - The last time we displayed an automated print-screen message to the client.
	clock_t		firstEnterTime;
	// Maker Mod members...
	//char		ip[MAX_IPSTRING];
} clientPersistant_t;
...
struct gclient_s {
	...
	int			godModFlags;
	int			manipulating;
	int         lastAttackYaw; // for preventing pokes( ie. wiggling/shaking saber from side to side during an attack)
    int         attackYawDirection; // which way we're rotating in yaw during attack
	int			lastYawDirChange; // time of the last yaw ang velocity direction change during an attack
	int         dirChangeCount; // for poke protection
	int			attackSamples; // Diagnostic test of how many samples of yaw we get in an attack
	int			attackExtreme;
	int         pokeRefundAmount[MAX_POKE_REFUNDS]; // damage refunds if an attack turns out to be a poke
	int         pokeRefundClient[MAX_POKE_REFUNDS];
	qboolean    pokeRefundDeath[MAX_POKE_REFUNDS];
	vec3_t      lastHitDirection; // remember this because we might have to die later
	vec3_t      lastHitSpot;
	vec3_t      lastDrawSpot;
	int			nextDrawTime;
	int			npcCount; // Number of npcs the person has. -Scooper
	char		npcModel[64]; //For npc anti-crash system.
	int			duelHealth; //Health at end of duel.
	int			selCount; // Same as manipulating, just if the object has a name.
	int			*selEnts;	// Pointer to array of selected targets
};
...
typedef struct {
...
#ifndef MM_RELEASE
	fileHandle_t	logFileAdmin;
	fileHandle_t	logFileComplain;
#endif
	...
	vec3_t      lastSaidOrigin;
	int			entCount; // count of entities..... num_entities above is a count of slots opened up
	int			npcCount; //How many npcs on server.
	int			npcTypes; //How many different npc types on server.
	int			allowedNPCs; //How many npcs each player can spawn. Calculated by limit/playing clients.
	qboolean	stripping;
	char		announcement[MAX_STRING_CHARS];
	int			announceStop; // When we should stop displaying the current announcement
	qboolean	fiveminutesWarning;
	qboolean	oneminuteWarning;
	qboolean	npcTypesFull;
	int			weatherLoc[18];	//Where the weather location in configstrings are.
	int			freezeLoc;	// So I don't have to lookup freeze each clearing.
} level_locals_t;
...
#define MM_ROTATE_STORAGE 128

#ifndef MM_RELEASE
//typedef unsigned int IP_t[4]; 
//extern IP_t;
#endif
#ifdef MM_BRUSH_OLD
...
//Makermod
char *G_NewStringM( const char *string,int ID );
char *G_NewString( const char *string );
char *G_NewStringForEnt ( const char* string, int entityNum );
...
void PerformPlayerResponses(); // Respond to previously issued
//Makermod
void ServerMessage( char *message, char *name );
extern void FreeSpawnVarsSlot(char* slot);
...
int		G_RemoveConfigstringIndex( const char *name, int start, int max, int id ); //Scooper
...
void	AddUse( gentity_t *ent, void (*use)(gentity_t *self, gentity_t *other, gentity_t *activator) );
void	RemoveUse(gentity_t *ent, void (*use)(gentity_t *self, gentity_t *other, gentity_t *activator) );
...
void G_PlayerOrigin(gentity_t *self, vec3_t *origin);
void AddRemap(const char *oldShader, const char *newShader, float timeOffset);
// Makermod
void ClearRemap();
void RemoveRemap(gentity_t *ent, const char *oldShader);
void RemoveRemapIndex(gentity_t *ent, int index);
void ListRemap(gentity_t *ent);
void UpdateXRemap(void); // Didz
// Makermod end
...
void SwapClippingBounds( vec3_t position );
...
//Makermod
void G_ReadNPCSessionData( void );
void G_WriteNPCSessionData( void );
...
#define MAX_MODEL_SIZES 640
#define MAX_SPAWN_VARS_SLOTS 640
#define MAX_SPAWN_VARS_LENGTH 255

extern int firstEmptySpawnVarsSlot;
extern  object_size_t modelSize[MAX_MODEL_SIZES] ;
extern	char		  spawnVars[MAX_SPAWN_VARS_SLOTS][MAX_SPAWN_VARS_LENGTH + 1];
extern	gentity_t		g_entities[MAX_GENTITIES];
//makermod
extern	npcTypes_t		npcTypesArray[14];
extern	mm_rotate_t		rotateStorage[MM_ROTATE_STORAGE];

extern int	maxRocketsPerLoop;
#define MM_MAX_ROCKET_EXPLOSIONS 128


extern qboolean		configSounds[MAX_SOUNDS];
extern qboolean		configModels[MAX_MODELS];
...
extern vmCvar_t		g_godAllowed;
extern vmCvar_t		g_allowMapVote;
extern vmCvar_t		g_defaultNewUserPermissions;
extern vmCvar_t		g_defaultNonUserPermissions;
extern vmCvar_t		g_forbiddenNPCs;
extern vmCvar_t		g_forbiddenVehicles;
extern vmCvar_t		g_forbiddenFXs;
extern vmCvar_t		g_objectMargin;
extern vmCvar_t		g_stripMaps;
extern vmCvar_t		g_spawnProtectDistance;
extern vmCvar_t		g_npcLimit;
extern vmCvar_t		g_antiNPCCrash;
extern vmCvar_t		g_OldLog;
extern vmCvar_t		g_teleDelay;
extern vmCvar_t		g_serverMessages;
#ifndef MM_RELEASE
extern vmCvar_t		g_ServerName;
#endif
extern vmCvar_t		g_antiChatFlood;
extern vmCvar_t		g_AllowBlackNames;
extern vmCvar_t		g_antiUserinfoFlood;
extern vmCvar_t		g_maxClientsfromIP;
extern vmCvar_t		g_JoinMOTD;
extern vmCvar_t		g_banRanges;
...
//void		Svcmd_Nav_f( void ); // Makermod
