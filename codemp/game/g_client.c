...
static void ClientCleanName( const char *in, char *out, int outSize ) {
	...
	*p = 0;
	...
	while( 1 ) {
		...
		//Scooper: Don't allow newline in name.
		if( ch == '\n' || ch == '\r') {
			continue;
		}

		//Scooper: Don't allow the .* namebug.
		/*if ( !*p && ch == '.') {
			if (*in == '*') {
				continue;
			}
		...
		//Scooper: Don't allow name with only colors or spaces.
		if (empty)
		{
			if (!(ch == '^') && !(ch == ' '))
				empty = qfalse;
		...
		//Scooper: Allow black names
		if (!g_AllowBlackNames.integer)
		{
	...
	if (precachedKyle && trap_G2_HaveWeGhoul2Models(precachedKyle))
	{
		...
		{ //rww - allow option for perplayer models on server for collision and bolt stuff.
			...
			if ( ent->client->NPC_class == CLASS_VEHICLE )
			{
				...
				//g_AntiNPCCrash Vehicle modelname
				Q_strncpyz(ent->client->npcModel, modelname,sizeof(ent->client->npcModel));
...
//	char	blueTeam[MAX_INFO_STRING];
	...
	if( g_antiUserinfoFlood.integer)
	{
		...
		if (ent->userinfoSpam > 7) {
			trap_SendServerCommand( ent-g_entities, va("print \"Userinfo changing too fast.\n\""));
			return;
		}
	...
	// Fix for values below 0x20. Up them to 0x20 instead. Hotfix
	for(i = 0; i < strlen(userinfo);i++)
		if(userinfo[i] < 32 && userinfo[i] > 0 );//userinfo[i] != 0)
			userinfo[i] = 32;

	// check for local client
//	s = Info_ValueForKey( userinfo, "ip" );

//	if ( strchr(s, '.') && strchr(s, ':') ) //expect IP addresses to include a dot. After the initial connection, this value gets blanked.
//		Q_strncpyz( client->sess.ip, s, MAX_IPSTRING );
	...
	if ( client->pers.connected == CON_CONNECTED ) {
		if ( strcmp( oldname, client->pers.netname ) ) 
		{
			...
			{				
				...
				G_LogPrintf( "print \"%s" S_COLOR_WHITE " %s %s\n\"", oldname, G_GetStringEdString("MP_SVGAME", "PLRENAME"), client->pers.netname );
...
//	char		*areabits;
	...
	int i;
	char PortlessIp[24];
	qboolean newIP = qtrue;
	qboolean isRange = qfalse;
#ifndef MM_RELEASE
//	IP_t ip[4];// = {0,0,0,0};
#endif
	...
	if(!userinfo || !userinfo[0])
		return "No userinfo detected, try restarting your game.";

#ifdef MM_BRUSH
	value = Info_ValueForKey (userinfo, "mm_client");
	if( (strcmp(value, MM_CLIENT_VERSION)) != 0 && !isBot)
	{
		return "You do not have the required plugin.\nServer requires version " MM_CLIENT_VERSION "\nPlease visit www.makermod.net";
	}
#endif

	value = Info_ValueForKey (userinfo, "ip");

	// Scooper - Max number of clients from 1 ip allowed on server:
	strcpy(PortlessIp, Info_ValueForKey (userinfo, "ip"));
	for( i = 0; i < 24; i++)
	{
		if (PortlessIp[i] == ':') {
			PortlessIp[i] = 0;
	...
	if ( (strcmp(value,"") == 0) && firstTime && !isBot)
		return "You don't get in this way! ;) -Scooper";

	// check to see if they are on the banned IP list
	if ( G_FilterPacket( value, &isRange ) ) {
		if(isRange)
		{
			G_LogPrintf( "RANGEBAN:Player %s, attempted to connect. With IP - %s \n",Info_ValueForKey(userinfo, "name"), Info_ValueForKey (userinfo, "ip"));
			return "Banned. Your IP is part of a range ban.";
		}
		else return "Banned.";
	}
	...
	if((strcmp(Info_ValueForKey(userinfo, "prx"),"") != 0))
		return "Access Denied.";

	//Detect fake players:
	if(gDetectFakePlayers)
	{
		if ((strcmp(Info_ValueForKey (userinfo, "cl_guid"),"") != 0) ||
			 (strcmp(Info_ValueForKey (userinfo, "cl_punkbuster"),"") != 0) ||
			 (strcmp(Info_ValueForKey (userinfo, "cl_anonymous"),"") != 0))
		{ // These are values set in the q3fill program, not something a normal player has.
			if (gFakePlayersBan)
			{
				G_LogPrintf("User has been denied connection, possibly fake client. IP: %s\n",
					PortlessIp);
				trap_SendConsoleCommand(EXEC_NOW, va("addip %s\n", PortlessIp));
				trap_SendServerCommand(-1, "print \"^7Connection declined, fake client detected. Auto-banned.\n\"");
				return "Nice try, you're banned from this server. Sincerely: Scooper";
			}
			else
			{
				G_LogPrintf("User has been denied connection, possibly fake client. IP: %s\n",
					PortlessIp);
				trap_SendServerCommand(-1, "print \"^7Connection declined, fake client detected.\n\"");
				return "Nice try, that won't work here. Sincerely: Scooper";			
			}
		}
	}
#ifndef MM_RELEASE
//	if(firstTime && !isBot)
//	{
	//	sscanf(PortlessIp, "%u.%u.%u.%u",&ip[0],&ip[1],&ip[2],&ip[3]);
...
//	areabits = client->areabits;
	...
	// [RemapObj]
	// makermod
	MMS_GetClientVersion(clientNum);
	// [RemapObj] end
	...
	else
	{
		if(!isBot)
		{
			G_ReadSessionData( ent->client, md5sum );
			// Reload the admin information, this is to prevent the password from being visible
			// in the session data.
			if(ent->client->sess.username && ent->client->sess.username[0])	// Hotfix
				ReadUserFile(ent, ent->client->sess.username, md5sum, qtrue);
			else ent->client->sess.permissions = StringToPermissionsNum( g_defaultNonUserPermissions.string );
			// End
		}
		else
	...
	if (firstTime)
	{
		...
		{
			for(i = 0; i < MAX_CLIENTS; i++)
				if (!cnctClients[i].numClientsFromIP)
					break;
			strcpy(cnctClients[i].PortlessIp, PortlessIp);
		}

		cnctClients[i].numClientsFromIP++;
		if (cnctClients[i].numClientsFromIP > g_maxClientsfromIP.integer) {
			cnctClients[i].numClientsFromIP--;
	...
	if ( !firstTime && !level.newSession )
	{	
		if ( *(client->sess.playerUsername) != 0 && strcmp(client->sess.playerUsername, ".") != 0 )
		{
			//tell our scoring dll that the session is continuing
			PlayerContinueSession( clientNum, client->sess.playerUsername );
		}
		else if ( client->sess.sessionTeam != TEAM_SPECTATOR )
		{
			trap_SendServerCommand( clientNum, "cp \"Not logged in. Type '/help' in the conosle for more info.\n\"" );
		}
	}
#endif
	...
	if ( firstTime || level.newSession )
	{
		trap_SendServerCommand( ent-g_entities, va("print \"-----------------------Welcome To Makermod-----------------------\n\""));
		trap_SendServerCommand( ent-g_entities, va("print \"This mod protects against poking (a saber exploit), lets people \n\""));
		trap_SendServerCommand( ent-g_entities, va("print \"build on the map and makes cheat servers safer.\n\n\""));
		trap_SendServerCommand( ent-g_entities, va("print \"Toast wrote it to celebrate our ability to learn to make things  \n\""));
		trap_SendServerCommand( ent-g_entities, va("print \"that improve our lives and the life of the species as a whole.   \n\""));
		trap_SendServerCommand( ent-g_entities, va("print \"Scooper has taken over coding for the mod, but continues with   \n\""));
		trap_SendServerCommand( ent-g_entities, va("print \"the same goals in mind as the original creator.   \n\""));
...
void ClientBegin( int clientNum, qboolean allowTeamReset ) {
	...
	if ( client->sess.sessionTeam != TEAM_SPECTATOR ) {
		...
		//trap_SendServerCommand( ent->s.clientNum, va("cp \"Welcome\n Please type /minfo in the console for help\n\"") );
	...
	// [RemapObject]
	// Makermod
	if(playerClients[clientNum].active && !playerClients[clientNum].numMessages && (!playerClients[clientNum].hasClient || playerClients[clientNum].version < MMS_VERSION))
	{
		//trap_SendServerCommand( clientNum, va("chat \"^7Welcome, there is a client plugin update available.\""));
		//trap_SendServerCommand( clientNum, va("chat \"^7You can download it at www.makermod.net, under the release category of the forum.\""));
		//trap_SendServerCommand( clientNum, va("chat \"^7For more information do /minfo update.\""));
		playerClients[clientNum].messageTime = level.time + 1000;
		playerClients[clientNum].numMessages++;
	}
	// [RemapObject] end
...
*/
	...
	{
		client->ps.stats[STAT_ARMOR] = client->ps.stats[STAT_MAX_HEALTH] * 0.25; //Instead of 0.25 -Scooper
	}
...
void ClientDisconnect( int clientNum ) {
	...
	// relinquish ownership of any npc's the client created. (for Maker Mod)
	// Erase objects created by the person leaving.
	for ( i = 0; i < ENTITYNUM_MAX_NORMAL; i++ )
	{
		...
		if ( ment->creator == clientNum + 1 )
		{
			ment->creator = 0;

			if( ment->s.eType != ET_NPC )
				G_FreeEntity( ment );
		}
	}
...
#ifdef RANKINGMOD
	// Inform our scoring DLL
	PlayerDisconnecting( clientNum );
#endif
