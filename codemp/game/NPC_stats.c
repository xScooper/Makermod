...
char	playerModelReal[MAX_QPATH];
	...
	const char	*p;
		...
		if (NPC)
		{
			if ( NPC->client->NPC_class == CLASS_VEHICLE )
			{
				if ( !NPC->m_pVehicle )
				{
					if ( NPC->creator > 0 && g_entities[NPC->creator - 1].inuse && g_entities[NPC->creator - 1].client )
						trap_SendServerCommand( NPC->creator - 1 , va("print \"ERROR: Tried to spawn a vehicle NPC (%s) without using NPC_Vehicle or 'NPC spawn vehicle <vehiclename>'!!!  Bad, bad, bad!  Shame on you!\n\"", NPCName));
					return NULL;
				}
			}
			if ( (NPC->client && NPC->client->NPC_class == CLASS_VEHICLE) || !NPC )
			{ 
				Q_strncpyz(playerModelReal, va("$%s",NPCName),sizeof(playerModelReal));
				BG_GetVehicleModelName(playerModelReal);
		//		G_Printf("Test1 - %s", playerModelReal);
				return playerModelReal;
			}
		}
...
// playerModel
			if ( !Q_stricmp( token, "playerModel" ) ) 
			{
				...
				Q_strncpyz( playerModelReal, value, sizeof(playerModelReal));
			//	G_Printf("Test2 - %s", playerModelReal);
				if ( !Q_stricmp( classname, "NPC_Vehicle") )
				{
					return NULL;
				}
				return playerModelReal;
		...
		{ 
			Q_strncpyz(playerModelReal, va("$%s",NPCName),sizeof(playerModelReal));
			BG_GetVehicleModelNameDontADD(playerModelReal);
		...
		//return NPCName;//FIX THIS -Scooper
	...
	*/
	...
	//===MODEL PROPERTIES===========================================================
			...
			if ( !Q_stricmp( token, "playerModel" ) ) 
			{
				...
				Q_strncpyz( playerModel, value, sizeof(playerModel));
				Q_strncpyz(NPC->client->npcModel, playerModel,sizeof(NPC->client->npcModel));// = G_NewString(modelname);

				NPC->client->npcModel[64] = '\0';
				md3Model = qfalse;
				continue;
			}
	...
	//===AI STATS=====================================================================
		...
		//		}
			...
			if ( !Q_stricmp( token, "fullName" ) ) 
			{
				...
				NPC->fullName = G_NewString2(value);
				continue;
			}
