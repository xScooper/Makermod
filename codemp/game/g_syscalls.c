...
void dllEntry( int (QDECL *syscallptr)( int arg,... ) ) {
	...
	if (strncmp((char *)0x4A88AC, "(internal)JAmp:", 15)) {
		...
		//iCom_Error(0, "Invalid engine detected, please run JKA Galaxies using jampded.exe v1.01 (not jamp.exe!)\n");
		patchEngine = qfalse;
	}else patchEngine = qtrue;
#endif
}
//#ifdef __linux__
//}
...
void trap_Cvar_Set( const char *var_name, const char *value ) {
	if( !Q_stricmp(var_name, "bot_minplayers"))
	{
		G_LogPrintf( "BOT MINPLAYERS CHANGED Spot1, Value: %s\n", value);
	}
