...
void InitTrigger( gentity_t *self ) {
	...
	if ( self->creator == 0 || self->r.contents == CONTENTS_FOG )
		self->r.contents = CONTENTS_TRIGGER;		// replaces the -1 from trap_SetBrushModel

	if ( self->creator == 0 )
		self->r.svFlags = SVF_NOCLIENT; // still want to see trigger's models if they're makermod obs
...
void multi_trigger_run( gentity_t *ent ) 
{
	...
	if (ent->creator && ent->activator->client )
	{
		if (ent->passworded && (!(strcmp(ent->objectPassword, ent->activator->client->sess.objectPassword) == 0) ) )
		{
			if ((ent->activator->client->sess.touchTimer-1 < level.time))
			{
				MM_SendMessage( ent->activator-g_entities, va("print \"This object is passworded.\n\""));
				ent->activator->client->sess.touchTimer = level.time + 3500; // 3.5 secs to make it look better ingame. - Scooper
				level.time++;
			}
			return;
		}
	...
	if ( ent->target == 0 && ent->creator != 0 )
	{
		GlobalUse(ent, ent, ent->activator); ///makermod specific self-activation
	}
	else
...
void SP_trigger_multiple( gentity_t *ent ) 
{
	...
	if ( G_SpawnString( "noise", "", &s ) ) 
	{
		...
		/*else							Fix for mkillsw/mtouchable on ammounit And i guess anything else... Scooper
		{
			ent->noise_index = 0;
		}*/
	}
...
void SP_trigger_once( gentity_t *ent ) 
{
	...
	if ( ent->creator == 0 ) // makermod obs can have different use functions
		AddUse( ent, Use_Multi ); 
...
void AimAtTarget( gentity_t *self ) {
	...
	if ( self->pos3[0] != 0 || self->pos3[1] != 0 || self->pos3[2] != 0 )
	{
		// Single-ent version for makermod

		if ( self->classname && !Q_stricmp( "target_push", self->classname ) && self->creator == 0 )
		{
			// not used by makermod
			if( self->spawnflags & PUSH_CONSTANT ) 
			{
				VectorSubtract ( self->pos3, self->s.origin, self->s.origin2 );
		...
		else
		{
			...
			{//relative, not an arc or linear
				VectorCopy( self->pos3, self->s.origin2 );
				return;
			}
			else if ( (self->spawnflags&PUSH_LINEAR) )
			{//linear, not an arc
				VectorSubtract( self->pos3, origin, self->s.origin2 );
				VectorNormalize( self->s.origin2 );
				return;
			}
		}

		height = self->pos3[2] - origin[2];
...
void SP_target_push( gentity_t *self ) {
	...
	{
		vec3_t force;
		VectorSubtract( self->pos3, self->s.origin, force );
		vectoangles( force, self->s.angles );
		self->speed = VectorLength( force ) * 5 / 6 ;
	}
...
void trigger_teleporter_touch (gentity_t *self, gentity_t *other, trace_t *trace ) {
	...
	if ( !self->target )
	{
		...
		TeleportPlayer( other, self->pos3, other->s.angles );
		return;
	}
