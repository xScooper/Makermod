...
//	gentity_t	*npcTypes = NULL;
	gentity_t	*creator = &g_entities[ent->creator-1];
	int			index, i = 0;
	vec3_t		saveOrg;
...
*/
	...
	/*if (level.npcTypesFull && g_antiNPCCrash.integer)
	{
		for (i = 0;i < 16;i++)
		{
			if( !Q_stricmp(npcTypesArray[i].NPCTypes, newent->client->npcModel))
				{
					return newent;
				}
		}
	...
	if( level.npcCount >= g_npcLimit.integer )
	{
		if( creator->client )
			trap_SendServerCommand( creator-g_entities, va("print \"Error: NPC limit hit. \n\"" ));
		return NULL;
	}

	if ( creator->client )
	{
		if ( creator->client->npcCount >= level.allowedNPCs )
		{
			trap_SendServerCommand( creator-g_entities, va("print \"Error: You have placed %i already. \n\"", creator->client->npcCount ));
			return NULL;
		}
	}
	...
	if( ent->count != -1 )
	{
		...
		if( ent->count <= 0 )
		{
			RemoveUse( ent, NPC_Spawn );//never again
...
//==NPC_Connect( newent, net_name );===================================
	...
	{
		ent->NPC_type = Q_strlwr( G_NewString2( ent->NPC_type ) );
	}
	...
	if ( Q_stricmp( ent->classname, "NPC_Vehicle" ) == 0 )
	{
 		...
 		if ( iVehIndex == VEHICLE_NONE )
		{
			...
			if ( newent->creator > 0 && g_entities[newent->creator - 1].inuse && g_entities[newent->creator - 1].client )
				trap_SendServerCommand( newent->creator - 1, va("print \"ERROR: Couldn't spawn NPC %s, it does not exist.\n\"", ent->NPC_type));
		...
		switch( g_vehicleInfo[iVehIndex].type )
		{
			...
			default:
				if ( newent->creator > 0 && g_entities[newent->creator - 1].inuse && g_entities[newent->creator - 1].client )
					trap_SendServerCommand( newent->creator - 1, va("print \"ERROR: Couldn't spawn NPC %s\n\"", ent->NPC_type));
			...
			/*	for (i = 0;i < 16;i++)
				...
				for (i = 0;i < 16;i++)
				{
					...
					{
						strcpy(npcTypesArray[i].NPCTypes, va("FailedNPC%i",i));//inewent->client->npcModel);
						if (i == 14) {
	...
	if ( ent->creator != 0 && (ent->pos3[0] != 0 || ent->pos3[1] != 0 || ent->pos3[2] != 0) )
	{
		// it's a makermod object with a special location for us to spawn to
		VectorCopy(ent->pos3, newent->s.origin);
		VectorCopy(ent->pos3, newent->client->ps.origin);
		VectorCopy(ent->pos3, newent->r.currentOrigin);
		G_SetOrigin(newent, ent->pos3);//just to be sure!
	}
	else
	{
		...
		if (!NPC_CheckNPCModel(ent->NPC_type, newent, NULL))
		{
			if ( newent->creator > 0 && g_entities[newent->creator - 1].inuse && g_entities[newent->creator - 1].client )
				trap_SendServerCommand( newent->creator - 1, va("print \"ERROR: Couldn't spawn NPC %s, it does not exist.\n\"", ent->NPC_type));
				G_FreeEntity( newent );
	...
	if ( !NPC_ParseParms( ent->NPC_type, newent ) )
	{
		...
		{
			if( !Q_stricmp(npcTypesArray[i].NPCTypes, va("%s ^1Failed",ent->NPC_type)))
				{
					G_FreeEntity( newent );
					G_FreeEntity( ent );
					return NULL;
		...
		for (i = 0;i < MAX_NPC_MODELS;i++)
		{
			if (!npcTypesArray[i].NPCTypes[0])
			{
				strcpy(npcTypesArray[i].NPCTypes, va("%s ^1Failed",ent->NPC_type));//inewent->client->npcModel);
...
//==New stuff=====================================================================
	...
	// makermod feature... respawn when they die
	if ( ent->closetarget && ent->creator != 0 )
...
finish:
	...
	for (i = 0;i < MAX_NPC_MODELS;i++)
	{
	//	G_LogPrintf("CrashPoint1 %i", i);
		if( !Q_stricmp(npcTypesArray[i].NPCTypes, newent->client->npcModel))
			{
			//	G_LogPrintf("1 - %s",newent->client->npcModel);
				return newent;
			}
		//G_LogPrintf("2 - %s",newent->client->npcModel);
	}
	...
	for (i = 0;i < MAX_NPC_MODELS;i++)
	{
		if (!npcTypesArray[i].NPCTypes[0])
		{
	//		G_LogPrintf("CrashPoint2 %i", i);
			strcpy(npcTypesArray[i].NPCTypes, newent->client->npcModel);
			if (i == MAX_NPC_MODELS-1) {
				level.npcTypesFull = qtrue;
	//			G_LogPrintf("CrashPoint3");
			}
			level.npcTypes++;
			return newent;
		}
...
void SP_NPC_spawner( gentity_t *self)
{
	...
	if ( self->creator > 0 ) //if makermod ob created by player (and not makermod ob auto-loaded with map)
	{
		// check that the npc isn't forbidden
		char	buffer[1024];
		char   *ptr, *token;

		Q_strncpyz( buffer, g_forbiddenNPCs.string, sizeof(buffer) );
		...
		{
			if ( stricmp(token, self->NPC_type) == 0 )
			{
				trap_SendServerCommand( self->creator - 1, va("print \"ERROR: NPC %s is not allowed\n\"", self->NPC_type));
				return;
			}
			token = COM_ParseExt(&ptr, qfalse);
		}
	...
	if ( self->creator != 0 && self->pos3[0] == 0 && self->pos3[1] == 0 && self->pos3[2] == 0 )
	{
		// makermod spawner spawn on top of themselves by default
		VectorCopy( self->s.origin, self->pos3 );
		self->pos3[2] += OFFSET_TO_FEET + self->r.maxs[2];
	}
	...
	*/
	...
	if ( self->targetname )
	{//Wait for triggering
		AddUse( self, NPC_Spawn );
	//	self->r.svFlags |= SVF_NPC_PRECACHE;//FIXME: precache my weapons somehow?

		if ( self->creator != 0 && strcmp(self->targetname, self->closetarget) == 0 )
		{
			// It's a makermod npc spawner that wants to make sure there's always an npc here.
			// Spawn an npc immediately to get us started.
			self->think = NPC_Spawn_Go;
			self->nextthink = level.time + START_TIME_REMOVE_ENTS + 50;
		}

		//NPC_PrecacheModels( self->NPC_type );a
	}
	else
	{
...
void SP_NPC_Vehicle( gentity_t *self)
{
	...
	if ( self->creator > 0 ) //if makermod ob created by player (and not makermod ob auto-loaded with map)
	{
		...
		Q_strncpyz( buffer, g_forbiddenVehicles.string, sizeof(buffer) );
		...
		while ( strlen(token) > 0 )
		{
			if ( stricmp(token, self->NPC_type) == 0 )
			{
				trap_SendServerCommand( self->creator - 1, va("print \"ERROR: Vehicle %s is not allowed\n\"", self->NPC_type));
				return;
			}
			token = COM_ParseExt(&ptr, qfalse);
		}
	...
	if ( self->targetname )
	{
		...
		AddUse( self, NPC_VehicleSpawnUse );

		if ( self->creator != 0 && strcmp(self->closetarget, self->targetname) == 0 )
		{
			// Makermod repeating spawner
			// Start us off with an object, without the need to be used
			G_VehicleSpawn( self );	
			return;
		}
	}
...
gentity_t *NPC_SpawnType( gentity_t *ent, char *npc_type, char *targetname, qboolean isVehicle ) 
{
	...
	if (level.npcTypesFull && g_antiNPCCrash.integer)
	{
		for (i = 0;i < MAX_NPC_MODELS;i++)
		{
			if( !Q_stricmp(npcTypesArray[i].NPCTypes, NPC_CheckNPCModel(npc_type, NULL, NULL)))
				{
					allowed = qtrue;
					break;
				}
		...
		if (!allowed)
		{
			if (ent->client)
			{
				trap_SendServerCommand( ent-g_entities, va("print \"There have already been spawned 14 different npcs, so these are your choices:\n\""));
				for (i = 0; i < MAX_NPC_MODELS; i++)
					trap_SendServerCommand( ent-g_entities, va("print \"Choice%i: %s \n\"",i+1, npcTypesArray[i].NPCTypes ));
				trap_SendServerCommand( ent-g_entities, va("print \"You can only spawn npcs using these models, until next map load, to prevent client crashes.\n\""));
			}
			return NULL;
		}
	}
	...
	if(!NPCspawner)
	{
		trap_SendServerCommand( ent-g_entities, va("print \"NPC_Spawn Error: Out of entities!\n\""));
	...
	if (!npc_type[0])
	{
		trap_SendServerCommand( ent-g_entities, va("print \"Error, expected one of:\n\""));
		trap_SendServerCommand( ent-g_entities, va("print \" NPC spawn [NPC type (from ext_data/NPCs)]\n\""));
		trap_SendServerCommand( ent-g_entities, va("print \" NPC spawn vehicle [VEH type (from ext_data/vehicles)]\n\""));
	...
	G_Printf( "mlog: NPC Spawn %s, V: %i, Client: %i - %s\n", npc_type, isVehicle, ent-g_entities, ent->client->pers.netname );


	if ( ent - g_entities >=0 && ent - g_entities < MAX_CLIENTS )
	{
		NPCspawner->creator = ent - g_entities + 1;

		//Com_Printf( S_COLOR_RED"created spawner\n" );
	}
	...
	NPCspawner->NPC_type = G_NewString2( npc_type );

	if ( targetname )
	{
		NPCspawner->NPC_targetname = G_NewString2(targetname);
	}
...
//	int i;
	...
	{
		if ( !CanExistHere(ent->r.currentOrigin, ENT_VEHICLE) )
		{
			trap_SendServerCommand( ent-g_entities, va("print \"Vehicles aren't allowed at this point on the map.\nPlease try somewhere else.\n\""));
			return;
		}
		...
		if ( !ent->client || !(ent->client->sess.permissions & PERMISSION_NPC_ADMIN) )
		{
			ptr = buffer;

			Q_strncpyz( buffer, g_forbiddenVehicles.string, sizeof(buffer) );
			
			// check the vehicle isn't forbidden
			token = COM_ParseExt(&ptr, qfalse);

			while ( strlen(token) > 0 )
			{
				if ( stricmp(token, npc_type) == 0 )
				{
					trap_SendServerCommand( ent-g_entities, va("print \"Vehicle %s is not allowed on this server\n\"", npc_type));
					return;
				}
				token = COM_ParseExt(&ptr, qfalse);
			}
	...
	{
		if ( !CanExistHere(ent->r.currentOrigin, ENT_NPC) )
		{
			trap_SendServerCommand( ent-g_entities, va("print \"NPC's aren't allowed at this point on the map.\nPlease try somewhere else.\n\""));
			return;
		}
	...
	//G_Printf("%s",NPC_CheckNPCModel(npc_type));
...
void NPC_Kill_f( gentity_t *ent ) 
{
	...
	qboolean	killEverything = qfalse;
	...
	if ( !name[0] )
	{
		trap_SendServerCommand( ent-g_entities, va("print \"Error, Expected:\n\""));
		trap_SendServerCommand( ent-g_entities, va("print \"NPC kill '[NPC targetname]' - kills NPCs with certain targetname\n\""));
		trap_SendServerCommand( ent-g_entities, va("print \"or\n\""));
		trap_SendServerCommand( ent-g_entities, va("print \"NPC kill 'all' - kills all NPCs\n\""));
		trap_SendServerCommand( ent-g_entities, va("print \"or\n\""));
		trap_SendServerCommand( ent-g_entities, va("print \"NPC team '[teamname]' - kills all NPCs of a certain team ('nonally' is all but your allies)\n\""));
...
//	if ( Q_stricmp( "all", name ) == 0 )
//	{
//		trap_SendServerCommand( clientNum, va("print \"NPC kill all - is not allowed on this server.\n\""));
//		return;
//	}

//	if ( Q_stricmp( "team", name ) == 0 )
//	{
//		trap_SendServerCommand( clientNum, va("print \"NPC kill team - is not allowed on this server.\n\""));
//		return;
//	}

	if ( Q_stricmp( "everything", name ) == 0 )
	{
		if ( !(ent->client->sess.permissions & PERMISSION_NPC_ADMIN) )
		{
			trap_SendServerCommand( clientNum, va("print \"You do not have permission to use this command\n\""));
			return;
		}
		else
		{
			...
			#ifndef MM_RELEASE
			G_LogAdminPrintf("Player: %s, AdminUser: %s, IP: %s, CMD: NPC KILL EVERYTHING\n\n",
	...
	if ( Q_stricmp( "team", name ) == 0 )
	{
		...
		if ( !name[0] )
		{
			trap_SendServerCommand( ent-g_entities, va("print \"NPC_Kill Error: 'npc kill team' requires a team name!\n\""));
			trap_SendServerCommand( ent-g_entities, va("print \"Valid team names are:\n\""));
			...
			trap_SendServerCommand( ent-g_entities, va("print \"nonally - kills all but your teammates\n\""));
		...
		else
		{
			...
			{
				trap_SendServerCommand( ent-g_entities, va("print \"NPC_Kill Error: team '%s' not recognized\n\""));
				trap_SendServerCommand( ent-g_entities, va("print \"Valid team names are:\n\""));
	...
	for ( n = 1; n < ENTITYNUM_MAX_NORMAL; n++) 
	{
		...
		if ( killEverything != qtrue && player->creator != 0 && player->creator != (clientNum + 1) )
			continue;
		if ( killNonSF )
		{
			if ( player )
		...
		else if ( player && player->NPC && player->client )
		{
			if ( killTeam != TEAM_FREE )
			{
				...
				|| (  Q_stricmp( name, "all" ) == 0 ) || killEverything == qtrue )
			{
				Com_Printf( S_COLOR_GREEN"Killing NPC %s named %s\n", player->NPC_type, player->targetname );
				player->health = 0;
...
void Cmd_NPC_f( gentity_t *ent ) 
{
	...
	if ( !cmd[0] ) 
	{
		trap_SendServerCommand( ent-g_entities, va("print \"Valid NPC commands are:\n\""));
		trap_SendServerCommand( ent-g_entities, va("print \" spawn [NPC type (from NCPCs.cfg)\n\""));
		trap_SendServerCommand( ent-g_entities, va("print \" kill [NPC targetname] or [all(kills all NPCs)] or 'team [teamname]\n\""));
		if (g_antiNPCCrash.integer)
			trap_SendServerCommand( ent-g_entities, va("print \" list, list of used NPC models.\n\""));
		if (level.npcTypesFull && g_antiNPCCrash.integer)
		{
			trap_SendServerCommand( ent-g_entities, va("print \"There have already been spawned 14 different kinds of npcs, so these are your choices:\n\""));
			for (i = 0; i < MAX_NPC_MODELS; i++)
				trap_SendServerCommand( ent-g_entities, va("print \"Choice%i: %s \n\"",i+1, npcTypesArray[i].NPCTypes ));
				trap_SendServerCommand( ent-g_entities, va("print \"You can only spawn npcs using these models, until next map load, to prevent client crashes.\n\""));
			return;
	...
	else if ( Q_stricmp( cmd, "list" ) == 0 )
	{
		if (g_antiNPCCrash.integer)
		{
			trap_SendServerCommand( ent-g_entities, va("print \"Here is a list of NPC models already used:\n\""));
			for (i = 0; i < MAX_NPC_MODELS; i++)
				if(npcTypesArray[i].NPCTypes[0]){
					trap_SendServerCommand( ent-g_entities, va("print \"Model%i: %s \n\"",i+1, npcTypesArray[i].NPCTypes ));
				}
				trap_SendServerCommand( ent-g_entities, va("print \"Some slots might be taken by npcs that have failed. \nUnmodified windows clients only support 15 different npc models, so the limit is set to 14.\n\""));
			return;
		}
		else
			trap_SendServerCommand( ent-g_entities, va("print \"This server does not use the Anti NPC Crash system. \n\""));
	}
	else if ( Q_stricmp( cmd, "spawn" ) == 0 )
	{
		if ( level.entCount > MAX_GENTITIES - g_objectMargin.integer )
		{
			trap_SendServerCommand( ent-g_entities, "print \"Error: There aren't enough free object slots\n\"" );
			return;
		}
		if ( level.npcCount >= g_npcLimit.integer )
		{
			trap_SendServerCommand( ent-g_entities, "print \"Error: No more NPC slots available.\n\"" );
			return;
		}
		if ( ent->client->npcCount >= level.allowedNPCs )
		{
			trap_SendServerCommand( ent-g_entities, va("print \"Error: You have placed %i already. \n\"", ent->client->npcCount ));
			return;
		}
