...
const char *G_RefreshNextMap(int gametype, qboolean forced)
{
...
#ifdef RANKINGMOD
	{
		...
		PlayerGetNextMap( mapname.string, nextMap, MAX_STRING_CHARS );
		trap_Cvar_Set( "nextmap", va("map %s", nextMap));
...
void G_CheckMinimumPlayers( void ) {
	...
	{
		G_LogPrintf( "BOT MINPLAYERS CHANGED Spot2, Integer: %i, ModifiedCount: %i\n", bot_minplayers.integer, bot_minplayers.modificationCount);
...
//	Info_SetValueForKey( userinfo, "characterfile", Info_ValueForKey( botinfo, "aifile" ) );
	...
	if (bot->client->sess.sessionTeam != preTeam)
	{
		...
		G_ReadSessionData( bot->client, NULL );
