...
int remapCount = 0;
qboolean remapsModified = qfalse;
shaderRemap_t remappedShaders[MAX_SHADER_REMAPS];


qboolean	configSounds[MAX_SOUNDS];
qboolean	configModels[MAX_MODELS];

void UpdateXRemap(void)
{
	int i;
	int j;
	...
	if ( remapsModified )
	{
		...
		// Remove remaps marked for deletion from the array
		for (i = 0; i < remapCount; ) {
			if ( remappedShaders[i].deletion ) {
				for (j = i; j < (remapCount-1); j++) { //this loop shifts all elements after the deleted remap down the array
					remappedShaders[j] = remappedShaders[j+1];
				}

				remapCount--; //then we lower the remap count
			} else {
				i++;
			}
		}
...
void AddRemap(const char *oldShader, const char *newShader, float timeOffset) {
	...
	remapsModified = qtrue;
	for (i = 0; i < remapCount; i++) {
		if (Q_stricmp(oldShader, remappedShaders[i].oldShader) == 0) {
			// found it, just update this one
			strcpy(remappedShaders[i].newShader,newShader);
			remappedShaders[i].timeOffset = timeOffset;
			remappedShaders[i].deletion = qfalse;
			return;
		}
	}
	if (remapCount < MAX_SHADER_REMAPS) {
		...
		remappedShaders[remapCount].deletion = qfalse;
		remapCount++;
	}
}
...
void ClearRemap()
{
	...
	for (i = 0; i < remapCount; i++) {
		AddRemap(remappedShaders[i].oldShader, remappedShaders[i].oldShader, f);
		remappedShaders[i].deletion = qtrue;
	}
...
void RemoveRemap(gentity_t *ent, const char *oldShader) {
	int i;
//	int j;
	float timeOffset = level.time * 0.001;

	for (i = 0; i < remapCount; i++) {
		if (Q_stricmp(oldShader, remappedShaders[i].oldShader) == 0) {
			// we found the remap, so we overwrite it to itself, then shift the array of remaps down
			AddRemap(oldShader, oldShader, timeOffset); //We must remap it to itself first so the client knows to change it back
		//	trap_SetConfigstring(CS_SHADERSTATE, BuildShaderStateConfig()); //Rebuilds the config string for shader state


			// New way of doing it, mark the shader for deletion by the frame code so we can deal with all
			// modified remaps in one shot rather than flooding clients with configstring changes.
			remappedShaders[i].deletion = qtrue;
			remapsModified = qtrue;
			return;
	...
	if(ent)
		trap_SendServerCommand( ent-g_entities, va( "print \"Could not find %s in remaps.\n\"", oldShader ) );
	else G_Printf("Could not find %s in remaps.\n",oldShader);
}

void RemoveRemapIndex(gentity_t *ent, int index)
{
	if ( index < remapCount  && index > 0) {
		RemoveRemap(ent, remappedShaders[index].oldShader);
	}
	else 
	{
		if(ent)
			trap_SendServerCommand( ent-g_entities, va( "print \"Invalid index chosen.\n\"" ) );
		else G_Printf("Invalid index chosen.\n");
	}
}
void ListRemap(gentity_t *ent)
{
	...
	{
		if(ent)
			trap_SendServerCommand( ent-g_entities, va( "print \"There are no remaps.\n\"" ) );
		else G_Printf("There are no remaps.\n");
		return;
	}

	if(ent)
		trap_SendServerCommand( ent-g_entities, va( "print \"Shader Remap List:\n\"" ) );
	else G_Printf("Shader Remap List:\n");
	if(ent)
	{
		for( i = 0; i < remapCount; i++) {
			trap_SendServerCommand( ent-g_entities, va( "print \"^3%i^7: %s ^3-> ^7%s\n\"",i, remappedShaders[i].oldShader, remappedShaders[i].newShader ) );
		}
	}else
	{
		for( i = 0; i < remapCount; i++) {
...
static int G_FindConfigstringIndex( const char *name, int start, int max, qboolean create ) {
	...
	for ( i=1 ; i<max ; i++ ) {
		...
		if( start == CS_MODELS )
		{
			if( !configModels[i] && !avail)
			{
				avail = i;
				continue;
			}
			if( avail && configModels[avail])
			{
				avail = 0;
				continue;
			}
		...
		{
			if( !configSounds[i] && !avail )
			{
				avail = i;
				continue;
			}
			if( avail && configSounds[avail])
			{
				avail = 0;
				continue;
			}
		...
		if ( !strcmp( s, name ) ) {
			...
			{
				if(!configModels[i])
					configModels[i] = qtrue;
			}
			else if ( start == CS_SOUNDS )
			{
				if(!configSounds[i])
					configSounds[i] = qtrue;
			}
	...
	if ( i == max ) {
		G_Error( "G_FindConfigstringIndex: overflow, %i, %s", max, name );
	}
...
================
*/
int G_RemoveConfigstringIndex( const char *name, int start, int max, int id ) {
	int		i;
	char	s[MAX_STRING_CHARS];
	...
	if( start == CS_EFFECTS )
	{
		trap_SetConfigstring( start + id, "");
	}
	else if( start == CS_SOUNDS )
	{
		configSounds[id] = qfalse;
	}
	else if( start == CS_MODELS )
	{
		configModels[id] = qfalse;
	}
	else return 0;
...
//	for ( i=0 ; i<POOLSIZE ; i++ ) {
		//trap_GetConfigstring( start + i, s, sizeof( s ) );
//		MemoryBuffer = 
		//	return G_FindAlloc(name,MemoryBuffer);
		/*if (MemoryBuffer[0])
		{
			G_Printf("Cheese2");
			//buffer = &MemoryBuffer;
...
void AddUse( gentity_t *ent, void (*use)(gentity_t *self, gentity_t *other, gentity_t *activator))
{
	...
	for( i = 0 ; i < MAX_EXTRA_USE ; i++ )
	{
		if ( ent->extraUse[i] == use )
			return;
	...
	G_Printf( "ERROR: Run out of extra use slots\n" );
}
...
void GlobalUse(gentity_t *self, gentity_t *other, gentity_t *activator)
{
	...
	if(!(!Q_stricmp(self->targetname,activator->target) && self->use == target_kill_use))		// mspawner mkillsw fix. Scooper
		self->use(self, other, activator);

	for ( i = 0 ; i < MAX_EXTRA_USE && self->extraUse[i] != 0 ; i++ )
		if(self->extraUse[i] && !(!Q_stricmp(self->targetname, activator->target) && self->extraUse[i] == target_kill_use))	// Anti loop -Scooper
			(self->extraUse[i])(self, other, activator);
}
...
void G_UseTargets2( gentity_t *ent, gentity_t *activator, const char *string ) {
	...
	while ( (t = G_Find (t, FOFS(targetname), string)) != NULL ) {
		if ( t == ent ) {
			...
			if ( t->use ) {//&& (Q_stricmp(t->targetname,activator->target) && t->use == target_kill_use)) {		// mspawner mkillsw fix. Scooper
...
char	*vtos( const vec3_t v ) {
	static	int		index;
	static	char	str[8][64];	// Changed from [8][32], to prevent frequent buffer overflows in com_sprintf. - Scooper
	char	*s;
	...
	Com_sprintf (s, 64, "(%i %i %i)", (int)v[0], (int)v[1], (int)v[2]);
...
static void G_SpewEntList(void)
{
	...
	str = va("TempEnt count: %i\nTempEnt ST: %i\nNPC count: %i\nProjectile count: %i\n Ent count: %i\n", numTempEnt, numTempEntST, numNPC, numProjectile, level.entCount);
	Com_Printf(str);
#ifndef VM_OR_FINAL_BUILD
	if (fh)
	{
...
gentity_t *G_Spawn( void ) {
	...
	for ( force = 0 ; force < 2 ; force++ ) {
		...
		for ( i = MAX_CLIENTS ; i<level.num_entities ; i++, e++) {
			...
			if ( !force && e->freetime > level.startTime + 2000 && level.time - e->freetime < 1000 && level.time < 7000 )
			{
				continue;
			}
	...
	if ( i == ENTITYNUM_MAX_NORMAL ) {
		...
		*/
		G_SpewEntList();
#ifndef MM_RELEASE
		G_Printf("Level EntCount: %i  Max: %i  Num_entities: %i \n", level.entCount, MAX_GENTITIES, level.num_entities);
#endif
...
//	char buffer[MAX_TOKEN_CHARS];
	...
	if(ed->s.eType == ET_FX /*&& ed->target6 && ed->target6[0]*/)
	{
		removeConfig = qtrue;
//		Q_strncpyz(buffer, ed->target6,sizeof(buffer));
		id1 = ed->s.modelindex;
		ed->s.modelindex = 0;
		removeType += 2;
	}
	...
	if(ed->s.eType == ET_MOVER)
	{
		id3 = ed->s.modelindex2;
		ed->s.modelindex2 = 0;
		removeConfig = qtrue;
		removeType += 8;
	}

	//Makermod: Memory freeing
	G_Free(ed->s.number);
	//Makermod: Spawnfield freeing
	if ( ed->spawnFields )
		FreeSpawnVarsSlot( ed->spawnFields );
	// [RemapObj]
	// Just to be sure, I want to reset the trickedindex stuff
	if(ed->s.trickedentindex)
	{
		ed->s.trickedentindex = 0;
		ed->s.trickedentindex2 = 0;
		ed->s.trickedentindex3 = 0;
	}
	// [RemapObj] - end
	//End of makermod - Scooper
	...
	freedClassname = "freed";

	if ( ed->classname != freedClassname )
		level.entCount--;

	memset (ed, 0, sizeof(*ed));
	ed->classname = freedClassname;
	...
	{
		if( removeType & 2 )
			RemoveConfig(id1, CS_EFFECTS, MAX_FX, 0);
		if( removeType & 4 )
			RemoveConfig(id2, CS_SOUNDS, MAX_SOUNDS, 0);
		if( removeType & 8 )
			RemoveConfig(id3, CS_MODELS, MAX_MODELS, 0);
	}
}
...
//Enable for corpse dragging
	...
	if (target->creator && (ValidUseTarget(target) || target->m_pVehicle))
	{
		if (target && target->passworded && (!(strcmp(target->objectPassword, ent->client->sess.objectPassword) == 0) ) )
		{
			MM_SendMessage( ent-g_entities, va("print \"This object is passworded.\n\""));
			return;
		}
	}

	//To fix error message in final build, don't think this is necessary -Scooper
	/*if(ValidUseTarget(target))
	{
		if(target->creator)
		{
			if((target->genericValue15-4000) > level.time)
			{
				trap_SendServerCommand( ent-g_entities, va("print \"Already been used, wait a little.\n\""));
				return;
			}
		}
	}*/
			...
			|| g_ff_objectives.integer) )
	{
		...
		*/
		...
		else
		{
			...
			if ( target->count && target->classname && (strcmp(target->classname, "misc_shield_floor_unit") == 0 || strcmp(target->classname, "misc_ammo_floor_unit") == 0) )
				ent->client->ps.useDelay = level.time - 100; // shorter bounce time for ammo and shield units
		}
		return;
	}
...
================
G_PlayerOrigin
Check if a player is at this origin
===============
*/
void G_PlayerOrigin (gentity_t *self, vec3_t *origin)
{
	...
	if(trace.contents == CONTENTS_BODY )
	{
		...
		*origin[2] -= self->r.mins[2];
		G_PlayerOrigin(self, origin);
	}
}
