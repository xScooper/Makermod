...
char  *DEFAULT_FORCE	= "7-1-033330000000003330";
//Max and min allowed settings:
char  *FORCE_LOWER 	= "0-1-000000000000000000";
char  *FORCE_UPPER 	= "7-2-333333333333333333";

char *CheckForceString(char* s) {
	char *p = s, 
		 *pu = FORCE_UPPER, 
		 *pl = FORCE_LOWER;
...
void WP_InitForcePowers( gentity_t *ent )
{
	...
	*/
	...
	// Forcestring crash -Scooper:
	temp = CheckForceString(forcePowers);
...
//		va("print \"^1Incorrect force string '%s'. Replaced by default.\n\"", forcePowers));
		...
		G_LogPrintf("FORCESTRING CRASH: Client num %d tried to take incorrect forcestring '%s'.",
...
qboolean WP_ForcePowerAvailable( gentity_t *self, forcePowers_t forcePower, int overrideAmt )
{
	...
	if ((forcePower == FP_DRAIN || forcePower == FP_LIGHTNING) &&
		(self->client->ps.fd.forcePower >= 25 || (self->client->godModFlags & GMOD_EMPOWERED)) )
	{ //it's ok then, drain/lightning are actually duration
		return qtrue;
	}
	if ( (self->client->ps.fd.forcePower < drain) && !(self->client->godModFlags & GMOD_EMPOWERED) )
	{
		return qfalse;
	}
	return qtrue;
...
qboolean WP_ForcePowerUsable( gentity_t *self, forcePowers_t forcePower )
{
	...
	if ( (!(self->client->ps.fd.forcePowersKnown & ( 1 << forcePower ))) && !(self->client->godModFlags & GMOD_EMPOWERED) )
	...
	if ((!self->client->ps.fd.forcePowerLevel[forcePower]) && !(self->client->godModFlags & GMOD_EMPOWERED) )
	{
		return qfalse;
	}
...
{ //activate the given force power
	...
	case FP_SPEED:
		hearable = qtrue;
		hearDist = 256;
		if ( self->client->godModFlags & GMOD_EMPOWERED )
		{
			duration = 20000;
		}
	...
	case FP_TELEPATHY:
		...
		else if (self->client->ps.fd.forcePowerLevel[FP_TELEPATHY] == FORCE_LEVEL_1)
		{
			duration = 20000;
		}
	...
	case FP_LIGHTNING:
		...
		self->client->ps.fd.forcePowersActive |= ( 1 << forcePower );
		if (self->client->godModFlags & GMOD_EMPOWERED)
			self->client->ps.activeForcePass = FORCE_LEVEL_3;
		else
	...
	case FP_SEE:
		...
		else if (self->client->ps.fd.forcePowerLevel[FP_SEE] == FORCE_LEVEL_1)
		{
			duration = 10000;
		}
...
void ForceHeal( gentity_t *self )
{
	...
	if (self->client->ps.fd.forcePowerLevel[FP_HEAL] == FORCE_LEVEL_3 || (self->client->godModFlags & GMOD_EMPOWERED) )
	{
...
void ForceTeamHeal( gentity_t *self )
{
	...
	if (self->client->ps.fd.forcePowerLevel[FP_TEAM_HEAL] == FORCE_LEVEL_3 || (self->client->godModFlags & GMOD_EMPOWERED))
	{
		radius *= 2;
	}
...
void ForceTeamForceReplenish( gentity_t *self )
{
	...
	if (self->client->ps.fd.forcePowerLevel[FP_TEAM_FORCE] == FORCE_LEVEL_3 || (self->client->godModFlags & GMOD_EMPOWERED))
	{
		radius *= 2;
	}
...
void ForceLightning( gentity_t *self )
{
	...
	if ( (self->client->ps.fd.forcePower < 25 && !(self->client->godModFlags & GMOD_EMPOWERED)) || !WP_ForcePowerUsable( self, FP_LIGHTNING ) )
	{
		return;
	}
		...
		{//an enemy or object
			...
			if (ForcePowerUsableOn(self, traceEnt, FP_LIGHTNING))
			{
				...
				if ( self->client->ps.weapon == WP_MELEE
					&& (self->client->ps.fd.forcePowerLevel[FP_LIGHTNING] > FORCE_LEVEL_2 || (self->client->godModFlags & GMOD_EMPOWERED)) )
				{//2-handed lightning
...
void ForceShootLightning( gentity_t *self )
{
	...
	if ( self->client->ps.fd.forcePowerLevel[FP_LIGHTNING] > FORCE_LEVEL_2 || (self->client->godModFlags & GMOD_EMPOWERED) )
	{//arc
...
void ForceDrain( gentity_t *self )
{
	...
	if ( (self->client->ps.fd.forcePower < 25 || !WP_ForcePowerUsable( self, FP_DRAIN )) && !(self->client->godModFlags & GMOD_EMPOWERED) )
	{
		return;
	}
	...
	if ( traceEnt && traceEnt->takedamage )
	{
		...
		{//an enemy or object
			...
			if (ForcePowerUsableOn(self, traceEnt, FP_DRAIN))
			{
				...
				else if (self->client->ps.fd.forcePowerLevel[FP_DRAIN] == FORCE_LEVEL_3 || (self->client->godModFlags & GMOD_EMPOWERED) )
				{
					dmg = 4;
				}
...
#ifdef MM_THROW2
			else if ( push_list[x]->s.eType == ET_FX && push_list[x]->thrown )
				PushThrow(self, push_list[x], forward);
#endif
...
static void WP_ForcePowerRun( gentity_t *self, forcePowers_t forcePower, usercmd_t *cmd )
{
	...
	case FP_GRIP:
		...
		if (self->client->ps.fd.forcePower < 1 && !(self->client->godModFlags & GMOD_EMPOWERED))
		{
			WP_ForcePowerStop(self, FP_GRIP);
			break;
		}
...
int WP_DoSpecificPower( gentity_t *self, usercmd_t *ucmd, forcePowers_t forcepower)
{
	...
	if ( self && self->client && (self->client->godModFlags & GMOD_EMPOWERED) )
		empowered = qtrue;
	else
		empowered = qfalse;

	// OVERRIDEFIXME
	if ( (empowered == qfalse) && (!WP_ForcePowerAvailable( self, forcepower, 0 )) )
	{
		return 0;
	}
