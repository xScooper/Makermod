...
static void CG_ServerCommand( void ) {
	...
	// [RemapObj]
	if ( !strcmp( cmd, "updateRemap" ) ) {
		ParseUpdateRemap();
		return;
	}
	if ( !strcmp( cmd, "clearRemap" )) {
		ParseClearRemap();
		return;
	}
	if ( !strcmp( cmd, "mmClientVersion" )) {
		MMC_ClientVersion();
		return;
	}
	if ( !strcmp( cmd, "mmEnableFeature" )) {
		MMC_EnableFeature();
		return;
	}
	// [RemapObj] end
	...
	if ( !strcmp( cmd, "chat" ) ) {
		if ( !cg_teamChatsOnly.integer ) {
			...
			CG_Printf( "*%s\n", text );
/*
			if(!Q_strncmp( text, va( "%s: !about"), cgs.clientinfo[cg.clientNum].name, strlen(cgs.clientinfo[cg.clientNum].name)+7))
				trap_SendConsoleCommand("say I said about!!");*/
		}
		return;
	}
