...
//	{ &cg_singlePlayerActive, "ui_singlePlayerActive", "0", CVAR_USERINFO},
	...
	//Makermod - Spinmod
	{ &mm_drawSpeed, "mm_drawSpeed", "0", CVAR_ARCHIVE  },
...
// Makermod v2.0 Global variables
#ifdef MM_BRUSH
...
void MM_ApplyMemory_f( void )
{
	oldAddresses[0] = (int)&cm->brushsides;
	cm->brushsides = brushsidesNew;
	oldAddresses[1] = (int)&cm->planes;
	cm->planes = planesNew;
	oldAddresses[2] = (int)&cm->leafs;
	cm->leafs = leafsNew;
	oldAddresses[3] = (int)&cm->leafbrushes;
	cm->leafbrushes = leafbrushesNew;
	oldAddresses[4] = (int)&cm->cmodels;
	cm->cmodels = cmodelsNew;
	oldAddresses[5] = (int)&cm->brushes;
	cm->brushes = brushesNew;
	oldAddresses[6] = (int)&(*box_planes);
	(*box_planes) = &cm->planes[cm->numPlanes];
	oldAddresses[7] = (int)&(*box_brush);
	(*box_brush) = &cm->brushes[cm->numBrushes];
}

void MM_FreeMemory_f( void )
{
	const char	*info;
	const char	*s;
	...
	if(strcmp(s,MM_GAMEVERSION))
		return;

	s = Info_ValueForKey( info, "mod_version" );
	if(strcmp(s,MM_MODVERSION))
		return;

	cm->brushsides = (cbrushside_t*)oldAddresses[0];
	cm->planes = (cplane_t*)oldAddresses[1];
	cm->leafs = (cLeaf_t*)oldAddresses[2];
	cm->leafbrushes = (int*)oldAddresses[3];
	cm->cmodels = (cmodel_t*)oldAddresses[4];
	cm->brushes = (cbrush_t*)oldAddresses[5];
	(*box_planes) = (cplane_t*)oldAddresses[6];
	(*box_brush) = (cbrush_t*)oldAddresses[7];
...
////	(*numSubModels3) += 1;
//
//	// Fix unknown model RGB axis:
//	cgs.inlineDrawModel[cm->numSubModels-1] = cgs.inlineDrawModel[1];
//
//	CG_Printf("Makermod - Brush model added.\n");
//}
...
qboolean MM_C_AddModel(const char *modelname)
{
	fileHandle_t fh;
	int len;
	byte *data;
...
void MM_FixMemory_f( void )
{
	...
	MM_ApplyMemory_f();
	CG_Printf("Makermod - Memory completed.\n");
	MM_AddBrushModel_f();
//	MM_C_AddModel("models/map_objects/korriban/torch_top.md3");
//	MM_C_AddModel("models/map_objects/factory/catw2_b.md3");
	MM_C_AddModel("models/map_objects/expansion/floor01.md3");
	...
	//	cm->brushes[i].contents = CONTENTS_WATER;
	...
	for(i = 0; i < cm->numSurfaces;i++)
	{
		...
		//cm->surfaces[i]->surfaceFlags |= SURF_SLICK;
	}
}
...
=================
*/
//void Init_UItoCG_Bridge(); // [UItoCG] - Scooper
void CG_Init( int serverMessageNum, int serverCommandSequence, int clientNum )
{
	static gitem_t *item;
	char buf[64];
...
//	CG_LoadingString( "graphics" );
	...
	// Makermod v2.0 This should be a good spot to fix memory
#ifdef MM_BRUSH
	MM_FixMemory_f();
#endif
...
//	CG_LoadingString( "Creating automap data" );
	...
	//Init_UItoCG_Bridge(); // [UItoCG] - Scooper
}
