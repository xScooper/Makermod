...
typedef struct fakeShader_t
{
	char name[MAX_QPATH];
	char filling[120];
	struct fakeShader_t *next;
}fakeShader_t;
...
void CG_TestStuff_f (void) {
	int i;
	int *count = (int*)0x00fe7428;
	fakeShader_t **shaders = (fakeShader_t**)0x00fe742c;
	model_t **models = (model_t**)0x00FE3C20;
	int *num_models = (int*)0x00FE4C20;
	md3Header_t *header = (md3Header_t*)models[144]->md3[0];
	...
	CG_Printf("Numshaders: %i\n", *count);

	for(i = 0; i < *count;i++)
	{
		//CG_Printf("Name: %s\n", shaders[i][0].name);
		if(strstr(shaders[i][0].name, "factory") != NULL)
		{
			CG_Printf("Found at %i - %s\n", i, shaders[i][0].name);
			//break;
		}
	}

	// Register new shaders, or just find the index handle by the below function
	// trap_R_RegisterShader("textures/factory/cat_floor_b");

	// md3Header_t *header = (md3Header_t*)models[145]->md3[0];
	// above line, the 145 number comes from:
	// handle_index = cgs.gameModels[ent->s.modelindex];


	//cgs.gameModels[cent->currentState.modelindex];

	//CG_Printf("next: %i\n", offsetof(fakeShader_t,next));
}


//int remapClientNum[1024];
//int remapObjectID[1024];
//int remapGroup[1024];

//int remapShaderFrom = 0;
//int remapShaderTo = 0;
//[MarkTest]
...
static void CG_UpdateThirdPersonTargetDamp(void)
{
	...
	{	
		...
		ratio = CG_powf(dampfactor, dtime);
