...
void CG_Special( centity_t *cent ) {
	...
	if ( s1->userInt1 )
	{
		char *path = s1->userInt2 ? "effects/env/portal1.efx" : "effects/env/portal2.efx";
		VectorMA( s1->origin, 64.0f, s1->boneAngles1, s1->angles );
		trap_FX_PlayEffectID( trap_FX_RegisterEffect( path ), s1->origin, s1->boneAngles1, -1, -1 );
...
extern int remappedObjectID;
extern int remappedObjectID2;
extern int *currentRenderObject;
int *numRenderObjects = 0x01053638;

//extern int remapClientNum;
//extern int remapObjectID;
//extern int remapGroup;

//extern int remapSurface;
//extern int remapTo;

#include "mmc_shaders.h"
extern RemapClientData remapClientData[1024];
//ShaderRemap entityShaderRemaps[1024];
int mm_numEntityShaderRemaps = 0;
// [RemapObject] end
...
*/
	...
	trap_R_AddRefEntityToScene(&ent);
	// [RemapObject]
	if(cent->currentState.trickedentindex == 1 && mm_EnableRemaps)
	{
		renderRemapped = qtrue;
		//remappedObjectID = *numRenderObjects;
		...
		remapClientData[*numRenderObjects].active = qtrue;
		remapClientData[*numRenderObjects].clientNum = cent->currentState.trickedentindex2;
		remapClientData[*numRenderObjects].group = cent->currentState.trickedentindex3;
		
		// Check if cached
		//for(i = 0; i < mm_numEntityShaderRemaps; i++)
		//{
		//	if(entityShaderRemaps[i].ownerNum == remapClientNum && entityShaderRemaps[i].objectID == cent->currentState.number)
		//	{
		//		// Found it!
		//		remapGroup = entityShaderRemaps[i].group;
		//		break;
		//	}
		//}
		//if(remapGroup == -1)
		////{
		//	entityShaderRemaps[mm_numEntityShaderRemaps].ownerNum = remapClientNum;
		//	entityShaderRemaps[mm_numEntityShaderRemaps].objectID = cent->currentState.number;
		//	entityShaderRemaps[mm_numEntityShaderRemaps].group = cent->currentState.trickedentindex3;
		//}
	}
	//else renderRemapped = qfalse;
	// [RemapObject] end
	...
	if ( s1->modelindex2 ) 
	{
		...
		trap_R_AddRefEntityToScene(&ent);
		// [RemapObject]
		//if(cent->currentState.trickedentindex == 2 && cent->currentState.trickedentindex2 == 1)
		//{
		//	renderRemapped = qtrue;
		//	remappedObjectID2 = *numRenderObjects;
		//}
		//else renderRemapped = qfalse;
		// [RemapObject] end
	}
